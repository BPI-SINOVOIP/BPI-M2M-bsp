/*
 * drivers/power/axp_power/axp259/axp259-regu.h
 *
...
#define AXP_LDO(_pmic, _id, min, max, step1, vreg,\
...
#define AXP_BUCK(_pmic, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level) \
{									\
	.desc	= {							\
		...
		.n_voltages = (step1) ? ((switch_vol) ? \
				((new_level) ? ((switch_vol - min) \
				/ step1 + (max - new_level) \
				/ step2  + 2) : ((switch_vol - min) \
				/ step1 + (max - switch_vol) \
				/ step2  + 1)) : ((max - min) \
				/ step1 + 1)) : 1,		\
	},								\
	.min_uV		= (min) * 1000,					\
...
#define AXP_DCDC(_pmic, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)	\
{									\
	.desc	= {							\
...
#define AXP_SW(_pmic, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level) \
{									\
	.desc	= {							\
