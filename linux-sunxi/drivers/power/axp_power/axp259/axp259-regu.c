...
static inline int
check_range(struct axp_regulator_info *info,
...
static int
axp_set_voltage(struct regulator_dev *rdev,
				int min_uV, int max_uV,
				unsigned *selector)
{
...
static struct regulator_ops axp259_ops = {
...
static struct regulator_ops axp259_ldoio01_ops = {
	.set_voltage = axp_set_voltage,
	.get_voltage = axp_get_voltage,
	.list_voltage = axp_list_voltage,
...
#define AXP22_LDO(_id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)	\
		AXP_LDO(AXP22, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)

#define AXP22_DCDC(_id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)	\
		AXP_DCDC(AXP22, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)

#define AXP22_SW(_id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)	\
		AXP_SW(AXP22, _id, min, max, step1, vreg,\
				shift, nbits, ereg, ebit, switch_vol,\
				step2, new_level)

static struct axp_regulator_info axp_regulator_info[] = {
	AXP22_LDO(1, AXP22LDO1, AXP22LDO1, 0, LDO1,
			0, 0, LDO1EN, 0, 0, 0, 0),/* ldo1 for rtc */
	AXP22_LDO(2, 700, 3300, 100, LDO2,
			0, 5, LDO2EN, 6, 0, 0, 0),/* ldo2 for aldo1 */
	AXP22_LDO(3, 700, 3300, 100, LDO3,
			0, 5, LDO3EN, 7, 0, 0, 0),/* ldo3 for aldo2 */
#ifdef CONFIG_AXP809
	AXP22_LDO(4, 700, 3300, 100, LDO4,
			0, 5, LDO4EN, 5, 0, 0, 0),/* ldo3 for aldo3 */
	AXP22_LDO(5, 700, 4200, 100, LDO5,
			0, 5, LDO5EN, 3, 3400, 200, 0),	/* ldo5 for dldo1 */
#else
	AXP22_LDO(4, 700, 3300, 100, LDO4,
			0, 5, LDO4EN, 7, 0, 0, 0),/* ldo3 for aldo3 */
	AXP22_LDO(5, 700, 3300, 100, LDO5,
			0, 5, LDO5EN, 3, 0, 0, 0),/* ldo5 for dldo1 */
#endif
	AXP22_LDO(6, 700, 3300, 100, LDO6,
			0, 5, LDO6EN, 4, 0, 0, 0),/* ldo6 for dldo2 */
	AXP22_LDO(7, 700, 3300, 100, LDO7,
			0, 5, LDO7EN, 5, 0, 0, 0),/* ldo7 for dldo3 */
	AXP22_LDO(8, 700, 3300, 100, LDO8,
			0, 5, LDO8EN, 6, 0, 0, 0),/* ldo8 for dldo4 */
	AXP22_LDO(9, 700, 3300, 100, LDO9,
			0, 5, LDO9EN, 0, 0, 0, 0),/* ldo9 for eldo1 */
	AXP22_LDO(10, 700, 3300, 100, LDO10,
			0, 5, LDO10EN, 1, 0, 0, 0),/* ldo10 for eldo2 */
	AXP22_LDO(11, 700, 3300, 100, LDO11,
			0, 5, LDO11EN, 2, 0, 0, 0),/* ldo11 for eldo3 */
	AXP22_LDO(12, 700, 3300, 100, LDO12,
			0, 3, LDO12EN, 0, 0, 0, 0),/* ldo12 for dc5ldo */
	AXP22_DCDC(1, 1600, 3400, 100, DCDC1,
			0, 5, DCDC1EN, 1, 0, 0, 0),/* buck1 for io */
	AXP22_DCDC(2, 600, 1540, 20, DCDC2,
			0, 6, DCDC2EN, 2, 0, 0, 0),/* buck2 for cpu */
	AXP22_DCDC(3, 600, 1860, 20, DCDC3,
			0, 6, DCDC3EN, 3, 0, 0, 0),/* buck3 for gpu */
	AXP22_DCDC(4, 600, 1540, 20, DCDC4,
			0, 6, DCDC4EN, 4, 0, 0, 0),/* buck4 for core */
	AXP22_DCDC(5, 1000, 2550, 50, DCDC5,
			0, 5, DCDC5EN, 5, 0, 0, 0),/* buck5 for ddr */
	AXP22_LDO(IO0, 700, 3300, 100, LDOIO0,
			0, 5, LDOIO0EN, 0, 0, 0, 0),/* ldoio0 */
	AXP22_LDO(IO1, 700, 3300, 100, LDOIO1,
			0, 5, LDOIO1EN, 0, 0, 0, 0),/* ldoio1 */
	AXP22_SW(1, 700, 3300, 100, DC1SW,
			0, 0, DC1SWEN, 7, 0, 0, 0),/* DC1SW */
};

static ssize_t
workmode_show(struct device *dev,
...
static ssize_t
workmode_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t count)
{
	...
	/* 1/AUTO: auto mode; 2/PWM: pwm mode; */
	if (buf[0] > '0' && buf[0] < '9')
		mode = buf[0];
	else
		mode = buf[1];
...
static ssize_t
frequency_show(struct device *dev,
...
static ssize_t
frequency_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct regulator_dev *rdev = dev_get_drvdata(dev);
...
static int axp_regulator_probe(struct platform_device *pdev)
{
	...
	if (ri->desc.id == AXP22_ID_DCDC1
		|| ri->desc.id == AXP22_ID_DCDC2
	    || ri->desc.id == AXP22_ID_DCDC3
		|| ri->desc.id == AXP22_ID_DCDC4
