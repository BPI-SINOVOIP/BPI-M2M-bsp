/*
 * drivers/power/axp_power/axp152_board.c
 *
...
#else
static int  axp152_platform_probe(struct platform_device *pdev)
{
	int ret = 0;

	axp152_dev.dev = &pdev->dev;
	dev_set_drvdata(axp152_dev.dev, &axp152_dev);
	axp152_dev.client = (struct i2c_client *)pdev;
	...
	ret = axp_register_mfd(&axp152_dev);
	if (ret < 0) {
		printk("axp152 mfd register failed\n");
		return ret;
	}
	return 0;
}

static struct platform_device axp152_platform_device = {
	.name		    = "axp15_board",
	.id		        = PLATFORM_DEVID_NONE,
};

static struct platform_driver axp152_platform_driver = {
	.probe		= axp152_platform_probe,
	.driver		= {
		.name	= "axp15_board",
		.owner	= THIS_MODULE,
	},
...
#else
		ret = platform_driver_register(&axp152_platform_driver);
		if (IS_ERR_VALUE(ret)) {
			printk("register axp152 platform driver failed\n");
			return ret;
		}
		ret = platform_device_register(&axp152_platform_device);
		if (IS_ERR_VALUE(ret)) {
			printk("register axp152 platform device failed\n");
			return ret;
		}
#endif
        } else {
...
subsys_initcall(axp152_board_init);
