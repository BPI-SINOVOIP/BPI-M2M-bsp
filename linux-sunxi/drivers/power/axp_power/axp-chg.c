		...
		if ((val & 0x02)) { /* usb and ac in short*/
			if((val & 0x10) == 0x00){/*usb or usb adapter can not be used*/
				...
				if(axp_config->pmu_usbcur_pc){
					axp_charger->chg_usb_ac_current_set(axp_charger, CHARGE_USB_20, axp_config->pmu_usbcur_pc);
				} else {
					axp_charger->chg_usb_ac_current_set(axp_charger, CHARGE_USB_20, 500);
				}
				...
				if(axp_config->pmu_usbcur){
					axp_charger->chg_usb_ac_current_set(axp_charger, CHARGE_USB_20, axp_config->pmu_usbcur);
				} else {
			...
			if(CHARGE_USB_20 == axp_usbvolflag){
				...
				if(axp_config->pmu_usbvol_pc){
					axp_charger->chg_usb_ac_vol_set(axp_charger, CHARGE_USB_20, axp_config->pmu_usbvol_pc);
				}
				...
				if(axp_config->pmu_usbvol){
					axp_charger->chg_usb_ac_vol_set(axp_charger, CHARGE_USB_20, axp_config->pmu_usbvol);
				}
			}
		}else {
