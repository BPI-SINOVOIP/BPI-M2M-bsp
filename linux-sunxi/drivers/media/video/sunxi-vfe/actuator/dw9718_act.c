...
#define SUNXI_ACT_NAME "dw9718_act"
...
#define act_dev_err(x,arg...) printk(KERN_ERR"[ACT_ERR][dw9718_act]"x,##arg)

//print unconditional, for important info
#if ACT_DEV_DBG_EN == 1
#define act_dev_dbg(x,arg...) printk(KERN_INFO"[ACT][dw9718_act]"x,##arg)
#else
                            ...
                            unsigned short halfword, void* pt)
{
	...
	cci_read_a8_d8(&act_ctrl->sdev, 0x10, &rdval);
	if(1 == (rdval&0x01))
	{		
	...
	ret = cci_write_a8_d8(&act_ctrl->sdev, 0x02, bdh&0x03);
	ret = cci_write_a8_d8(&act_ctrl->sdev, 0x03, bdl);
                       ...
                       struct actuator_para_t *a_para)
{
	...
	cci_write_a8_d8(&act_ctrl->sdev, 0x00, 0x01); // Power down mode (active high)
	cci_write_a8_d8(&act_ctrl->sdev, 0x00, 0x00); // Normal operation mode
	usleep_range(10000,12000);
	cci_write_a8_d8(&act_ctrl->sdev, 0x01, 0x08); // SAC3 mode, 
	cci_write_a8_d8(&act_ctrl->sdev, 0x05, 0x04); // DIV(Tvib x 2) ,Tvib= 13.4ms( 6.7ms x 2)
                       ...
                       unsigned short mode)
{
	...
	if(mode == 1)
	{
		...
		ret = subdev_set_code(act_ctrl,act_ctrl->active_min, 0);
		usleep_range(10000,12000);
		ret = cci_write_a8_d8(&act_ctrl->sdev, 0x00, 0x01); // Power down mode (active high)		
