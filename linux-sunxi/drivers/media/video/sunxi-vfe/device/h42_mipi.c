...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for h42_mipi Raw sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[h42_mipi Raw]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[h42_mipi Raw]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[h42_mipi Raw]"x,##arg)
...
/*
 * The h42_mipi i2c address
 */
#define I2C_ADDR 0x60
#define  SENSOR_NAME "h42_mipi"
...
//	printk("h42_mipi sensor_set_exp = 0x%x  0x%x, Done!\n", expmid, explow);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
  ...
  vfe_dev_dbg("h42_mipi ID_VAL_HIGH = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_HIGH)
    return -ENODEV;
 

  LOG_ERR_RET(sensor_read(sd, ID_REG_LOW, &rdval))
  vfe_dev_dbg("h42_mipi ID_VAL_LOW = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_LOW)
    return -ENODEV;
