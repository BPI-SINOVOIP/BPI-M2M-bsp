...
#define DEV_DBG_EN             0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) printk(KERN_INFO "[CSI_DEBUG][GC0308]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) printk(KERN_INFO "[CSI_ERR][GC0308]"x, ##arg)
#define vfe_dev_print(x, arg...) printk(KERN_INFO "[CSI][GC0308]"x, ##arg)
...
static int sensor_s_flip(struct v4l2_subdev *sd, int value)
{
	...
	case 1:
		val = value;
		ret = sensor_s_vflip(sd, val);
		break;
	case 2:
	case 3:
		val = value - 2;
		ret = sensor_s_hflip(sd, val);
		break;
	default:
		return -EINVAL;
	}
	if (ret < 0) {
		vfe_dev_err("sensor_write_gc0308 err at sensor_s_flip!\n");
		return ret;
	}
	return 0;
}
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	sensor_write_array(sd, sensor_default_regs_24M,
