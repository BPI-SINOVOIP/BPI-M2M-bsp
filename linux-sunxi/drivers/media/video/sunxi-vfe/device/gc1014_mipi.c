 ...
 */
/*
 * A V4L2 driver for gc1014_mipi Raw cameras.
 *
 */
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for gc1014_mipi Raw sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      0 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[gc1014_mipi Raw]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[gc1014_mipi Raw]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[gc1014_mipi Raw]"x,##arg)
...
#define V4L2_IDENT_SENSOR  0x1014
...
/*
 * The gc1014_mipi i2c address
 */
#define I2C_ADDR 0x78
#define  SENSOR_NAME "gc1014_mipi"
...
static struct regval_list sensor_default_regs[] = 
{
	 ...
	 {0xf8,0x03}, //Pll mode 2  // PCLK=48Mhz
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	//printk("gc1014_mipi sensor_set_exp = %d, Done!\n", exp_val);
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	unsigned int Analog_Multiple[9]={1000, 1706, 2000, 3414, 4026,6870,8127,13869,19610}; 
	unsigned int Analog_Index,Digital_Gain,Decimal;
	unsigned int All_gain;
		
	struct sensor_info *info = to_state(sd);
	//	printk("gc1014 sensor gain value is %d\n",gain_val);
...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, struct sensor_exp_gain *exp_gain)
{
  ...
  //printk("gc1014 sensor read out exp & gain is %d\t%d\n",exp_val, gain_val);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
  ...
  printk("gc1014_mipi ID_VAL_HIGH = %2x, Done!  20150316a \n", rdval);
  if(rdval != ID_VAL_HIGH)
    return -ENODEV;
 

  LOG_ERR_RET(sensor_read(sd, ID_REG_LOW, &rdval))
  //  printk("gc1014_mipi ID_VAL_LOW = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_LOW)
    return -ENODEV;
...
static struct sensor_win_size sensor_win_sizes[] = {
    {
        ...
        .vts        = 768,//814,
        .pclk       = 48*1000*1000,
        .mipi_bps	= 384*1000*1000,// 1lane mipi_bps=384*1000*1000 ;// 1lane mipi_bps=192*1000*1000
        .fps_fixed  = 1,
        .bin_factor = 1,
        .intg_min   = 1<<4,
        .intg_max   = 768<<4,// 814
