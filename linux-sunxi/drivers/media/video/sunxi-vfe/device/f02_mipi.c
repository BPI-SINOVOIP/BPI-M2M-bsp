 ...
 */
/*
 * A V4L2 driver for F02_mipi Raw cameras.
 *
 */
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for F02_mipi Raw sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[F02_mipi Raw]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[F02_mipi Raw]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[F02_mipi Raw]"x,##arg)
...
#define V4L2_IDENT_SENSOR  0x0F02
...
/*
 * The f02_mipi i2c address
 */
#define F02_WRITE_ADDR (0x80)
#define F02_READ_ADDR  (0x81)

//static struct delayed_work sensor_s_ae_ratio_work;
static struct v4l2_subdev *glb_sd;
#define SENSOR_NAME "f02_mipi"
...
//	printk("f02_mipi sensor_set_exp = %d Done!\n", exp_val );
...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, struct sensor_exp_gain *exp_gain)
{
  ...
  printk("f02 sensor exp & gain is %d\t%d\n", exp_val, gain_val);
  ...
  LOG_ERR_RET(sensor_read(sd, 0x1f, &rdval))
  rdval &= 0x7F;
  sensor_write(sd, 0x1f, rdval);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
  ...
  vfe_dev_dbg("f02_mipi ID_VAL_HIGH = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_HIGH)
    return -ENODEV;
 

  LOG_ERR_RET(sensor_read(sd, ID_REG_LOW, &rdval))
  vfe_dev_dbg("f02_mipi ID_VAL_LOW = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_LOW)
    return -ENODEV;
