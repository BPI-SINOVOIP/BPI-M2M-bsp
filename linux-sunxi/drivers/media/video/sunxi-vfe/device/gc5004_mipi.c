...
//	vfe_dev_dbg("sensor_set_exposure = %d\n", exp_val>>4);
	...
	//printk("gc5004_mipi sensor_set_exp = %d, Done!\n", exp_val);
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	else if((ANALOG_GAIN_1<= gain_val)&&(gain_val < ANALOG_GAIN_2))
	{
		...
		//printk("GC5004_mipi analogic gain 1x , GC5004_mipi add pregain = %d\n",tmp);
	}
	else if((ANALOG_GAIN_2<= gain_val)&&(gain_val < ANALOG_GAIN_3))
	{
		...
		sensor_write(sd, 0xb1, tmp>>6);
		sensor_write(sd, 0xb2, (tmp<<2)&0xfc);
		//printk("GC5004_mipi analogic gain 1.45x , GC5004_mipi add pregain = %d\n",tmp);
	}
	...
	else if((ANALOG_GAIN_3<= gain_val)&&(gain_val < ANALOG_GAIN_4))
	{
		...
		sensor_write(sd, 0xb1, tmp>>6);
		sensor_write(sd, 0xb2, (tmp<<2)&0xfc);
		//printk("GC5004_mipi analogic gain 2.02x , GC5004_mipi add pregain = %d\n",tmp);
	}
	else if((ANALOG_GAIN_4<= gain_val)&&(gain_val < ANALOG_GAIN_5))
	{
		...
		sensor_write(sd, 0xb1, tmp>>6);
		sensor_write(sd, 0xb2, (tmp<<2)&0xfc);
		//printk("GC5004_mipi analogic gain 2.86x , GC5004_mipi add pregain = %d\n",tmp);
	}

	//else if((ANALOG_GAIN_5<= gain_val)&&(gain_val)&&(gain_val < ANALOG_GAIN_6))
	else if(ANALOG_GAIN_5<= gain_val)
	{
		...
		sensor_write(sd, 0xb1, tmp>>6);
		sensor_write(sd, 0xb2, (tmp<<2)&0xfc);
		//printk("GC5004_mipi analogic gain 3.95x , GC5004_mipi add pregain = %d\n",tmp);
	}
...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, struct sensor_exp_gain *exp_gain)
{
  ...
  printk("gc2235 sensor read out exp & gain is %d\t%d\n",exp_val, gain_val);
...
//	printk("gc5004_mipi ID_VAL_HIGH = %2x, Done!\n", rdval);

  LOG_ERR_RET(sensor_read(sd, ID_REG_LOW, &rdval))
  if(rdval != ID_VAL_LOW)
    return -ENODEV;
  
//	printk("gc5004_mipi ID_VAL_LOW = %2x, Done!\n", rdval);
