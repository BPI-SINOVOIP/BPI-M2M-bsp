...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) printk(KERN_DEBUG"[H42 Raw]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) printk(KERN_ERR"[H42 Raw]"x, ##arg)
#define vfe_dev_print(x, arg...) printk(KERN_INFO"[H42 Raw]"x, ##arg)
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	vfe_dev_dbg("sensor_s_exp, expmid = 0x%x, explow = 0x%x\n",
			expmid, explow);
				...
				struct sensor_exp_gain *exp_gain)
{
	...
	vfe_dev_dbg("h42 set exp = %d, gain = %d\n", exp_val, gain_val);
...
static int sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	case CSI_SUBDEV_STBY_ON:
		...
		*/
		/*
		 * make sure that no device can access i2c bus
		 * during sensor initial or power down
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (rdval != ID_VAL_HIGH) {
		vfe_dev_dbg("*********sensor error,read id is %d.\n", rdval);
		return -ENODEV;
	}
	...
	if (rdval != ID_VAL_LOW) {
		...
		vfe_dev_dbg("*********find h42 raw data camera sensor now.\n");
		return 0;
	}
}
			...
			struct v4l2_mbus_framefmt *fmt)
{
	...
	*/
	...
	LOG_ERR_RET(
		sensor_write_array(sd, sensor_fmt->regs,
	...
	if (wsize->regs) {
		...
		LOG_ERR_RET(
			sensor_write_array(sd, wsize->regs,
						wsize->regs_size))
	}
