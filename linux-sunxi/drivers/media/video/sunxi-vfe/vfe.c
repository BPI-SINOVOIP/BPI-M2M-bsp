...
static ssize_t csi_isp_reset_en_show(struct device *dev,
		    struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", csi_isp_reset_en);
}

static ssize_t csi_isp_reset_en_store(struct device *dev,
		    struct device_attribute *attr,
		    const char *buf, size_t count)
{
	int err;
	...
	csi_isp_reset_en = val;
	vfe_print("csi_isp_reset_en = %ld\n", val);
...
static DEVICE_ATTR(csi_isp_reset_en, S_IRUGO|S_IWUSR|S_IWGRP,
		csi_isp_reset_en_show, csi_isp_reset_en_store);
...
static struct attribute *vfe_attributes[] = {
	...
	&dev_attr_csi_isp_reset_en.attr,
...
static enum v4l2_mbus_pixelcode try_yuv422_bus[] = {
	...
	V4L2_MBUS_FMT_VYUY8_16X1,
	V4L2_MBUS_FMT_YUYV8_16X1,
	V4L2_MBUS_FMT_YVYU8_16X1,
	...
	if(dev->is_bayer_raw) {
		...
		if(1 == isp_reparse_flag)
		{
			...
			if(read_ini_info(dev,dev->input, ReparseIniPath))
			{
				...
				/*
				 * whatever isp config reparse is ok or not,
				 * must call xx_init methods to prevent
				 * invalid parameters causing isp crash!
				 */
...
unlock:
							...
							&& dev->is_isp_used && bsp_isp_get_irq_status(FINISH_INT_EN))
	{
		//if(bsp_isp_get_para_ready())
		{
			...
			queue_work(dev->vfe_wq, &dev->isp_isr_bh_task);
...
static enum pkt_fmt get_pkt_fmt(enum bus_pixelcode bus_pix_code)
{
  switch(bus_pix_code) {
    ...
    case BUS_FMT_VYUY8_16X1:
    case BUS_FMT_YUYV8_16X1:
    case BUS_FMT_YVYU8_16X1:
      return MIPI_YUV422;
...
//      printk("dev->mipi_fmt.packet_fmt[%d] = %x\n",i,dev->mipi_fmt.packet_fmt[i]);
  ...
  if(dev->is_isp_used) {
	...
	vfe_print("[ISP] full_size width = %d, height = %d.\n",
					size_settings.full_size.width,
					size_settings.full_size.height);
	vfe_print("[ISP] scale_size width = %d, height = %d.\n",
					size_settings.scale_size.width,
					size_settings.scale_size.height);
	vfe_print("dev->buf_byte_size = %d, double_ch_flag = %d\n",
					dev->buf_byte_size,
					dev->isp_gen_set_pt->double_ch_flag);
  } else {
...
static int vfe_open(struct file *file)
{
	...
	if(dev->ccm_cfg[0]->is_isp_used || dev->ccm_cfg[1]->is_isp_used) {
		...
		dev->vfe_wq = create_singlethread_workqueue("vfe_wq");
		if (!dev->vfe_wq)
			vfe_err("create workqueue error at %s\n", __func__);
...
static int vfe_close(struct file *file)
{
	...
	if(dev->ccm_cfg[0]->is_isp_used || dev->ccm_cfg[1]->is_isp_used) {
		flush_workqueue(dev->vfe_wq);
		flush_work(&dev->isp_isr_set_sensor_task);
		if (dev->vfe_wq)
			destroy_workqueue(dev->vfe_wq);
