/*
 * drivers/video/sunxi/disp/de/lowlevel_sun8iw5/de_layer.c
 *
...
__u32  DE_BE_Addr_To_Offset(__u32 src_addr,__u32 off_addr,__u32 width,__u32 bpp,disp_position *pos)
{
    __u32    dist;
...
__s32 DE_BE_Layer_Set_Work_Mode(__u32 sel, __u8 layidx,__u8 mode)
{
    ...
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xff3fffff)|(mode&3)<<22);
...
__s32 DE_BE_Layer_Set_Format(__u32 sel, __u8 layidx,__u8 format, bool br_swap,__u8 order, bool pre_multiply)
{
    __u32 tmp;

    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF1,layidx);
    tmp &= 0x83f;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF1,layidx,(tmp&0xfffff000)|((format&0xf)<<8)|((br_swap&0x1)<<2)|(order&0x3));
...
__s32 DE_BE_Layer_Video_Enable(__u32 sel, __u8 layidx, bool video_en)
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xfffffffd)|(video_en&1)<<1);
...
__s32 DE_BE_Layer_Video_Ch_Sel(__u32 sel, __u8 layidx, bool scaler_index)
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xffffffef)|(scaler_index&1)<<4);
...
__s32 DE_BE_Layer_Yuv_Ch_Enable(__u32 sel, __u8 layidx, bool yuv_en)
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xfffffffb)|(yuv_en&1)<<2);
...
__s32 DE_BE_Layer_Set_Prio(__u32 sel, __u8 layidx,__u8 prio)
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xfffff3ff)|(prio&3)<<10);
...
__s32 DE_BE_Layer_Set_Pipe(__u32 sel, __u8 layidx,__u8 pipe)
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0xffff7fff)|(pipe&1)<<15);
...
__s32 DE_BE_Layer_Set_Alpha_Value(__u32 sel, __u8 layidx,__u8 alpha_val)//todo,why???
{
    ...
    tmp = DE_BE_RUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx);
    tmp &= 0xfffc8c17;//Invalid bits set to zero
    DE_BE_WUINT32IDX(sel, DE_BE_LAYER_ATTRCTL_OFF0,layidx,(tmp&0x00ffffff)|(alpha_val&0xff)<<24);
