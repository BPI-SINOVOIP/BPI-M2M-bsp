...
struct hwc_dispc_data {
    disp_layer_config       *hwc_layer_info[DISP_NUMS_SCREEN];
...
static int hwc_commit(void * user_display)
{
    ...
    {
         if(copy_from_user(&wb_data, (void __user *)disp_data.data, sizeof(disp_capture_info)))
        {
    ...
    for(i = 0; i < DISP_NUMS_SCREEN; i++)
    {
		...
		{
            if(copy_from_user(composer_priv.tmp_hw_lyr, (void __user *)disp_data.hwc_layer_info[i], sizeof(disp_layer_config)*(i?8:16)))
            {
...
s32 composer_init(disp_drv_info *psg_disp_drv)
{
    ...
    composer_priv.tmp_hw_lyr = kzalloc(sizeof(disp_layer_config)*16,GFP_KERNEL);
