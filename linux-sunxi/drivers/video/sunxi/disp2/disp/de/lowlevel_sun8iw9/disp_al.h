/*
 * drivers/video/sunxi/disp2/disp/de/lowlevel_sun8iw9/disp_al.h
 *
...
/* The unit of the clock rates below are hz.
 */
struct lcd_clk_info {
	disp_lcd_if lcd_if;
	unsigned long pll_rate; //clock rate of clock parent
	unsigned long tcon_rate; //clock rate of tcon
	unsigned long tcon_inner_div;//the inner clock divider of tcon
	unsigned long dclk_rate;//pixel clock rate
	unsigned long dsi_rate;//clock rate of dsi controller
	unsigned long dsi_phy_rate;//clock rate of dsi phy
};
