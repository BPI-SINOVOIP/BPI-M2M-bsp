					...
					scaler_para *ypara,scaler_para *cpara, unsigned char yv12_4k_en, unsigned char yv12_d1_en)
{
	...
	lan3coefftab32_left+pt_coef, sizeof(unsigned int)*VSU_PHASE_NUM);
	memcpy((void *)&vsu_dev[sel][chno]->yhcoeff1,
	lan3coefftab32_right+pt_coef, sizeof(unsigned int)*VSU_PHASE_NUM);

	pt_coef = de_vsu_calc_fir_coef(ypara->vstep);
	memcpy((void *)&vsu_dev[sel][chno]->yvcoeff,
	lan2coefftab32+pt_coef, sizeof(unsigned int)*VSU_PHASE_NUM);
