...
hist_data *hist_res[DEVICE_NUM][CHN_NUM];
...
int de_fce_init(unsigned int sel, unsigned int chno, unsigned int reg_base)
{
	...
	hist_res[sel][chno] = disp_sys_malloc(sizeof(hist_data));
	if(NULL == hist_res[sel][chno]) {
		__wrn("malloc hist_res[%d][%d] memory fail! size=0x%x\n", sel, chno, sizeof(hist_data));
		return -1;
	}
...
int toupdate(int hist_mean, int hist_diff, int diff_coeff, int change_thr)
{
  int toupdate=0;
  int diff_div=1;
						...
						unsigned char celut[256], hist_data* p_hist_data)
{
	...
	__u32 i;
	__u32 mean;
	__u32 total_pixel, total_pixel_r, total_size;
	__s32 uthr, lthr;
	__u32 rate, half;
	int tmp;
