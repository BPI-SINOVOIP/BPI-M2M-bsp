/*
 * drivers/video/sunxi/disp2/disp/de/disp_lcd.c
 *
...
#if !defined(CLK_FLEXIBLE)
static s32 lcd_clk_config(struct disp_device* lcd)
{
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
	struct lcd_clk_info clk_info;
	unsigned long pll_rate = 0, lcd_rate = 0, dclk_rate = 0, dsi_rate = 0;//hz
	unsigned long pll_rate_set = 0, lcd_rate_set = 0, dclk_rate_set = 0, dsi_rate_set = 0;//hz
...
{
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
	struct lcd_clk_info clk_info, clk_info_temp;
	struct lcd_clk_info *clk_info_set = &clk_info;
	...
	memset(&clk_info, 0, sizeof(struct lcd_clk_info));
	memset(&clk_info_temp, 0, sizeof(struct lcd_clk_info));
	disp_al_lcd_get_clk_info(lcd->disp, &clk_info, &lcdp->panel_info);

	disp_sys_clk_set_rate(lcdp->clk_parent, clk_info.pll_rate);
	clk_info_temp.pll_rate = disp_sys_clk_get_rate(lcdp->clk_parent);

	if (clk_info_temp.pll_rate != clk_info.pll_rate) {
		disp_al_lcd_get_clk_info(lcd->disp, &clk_info_temp, &lcdp->panel_info);
		clk_info_set = &clk_info_temp;
	}

	disp_sys_clk_set_rate(lcdp->clk, clk_info_set->tcon_rate);
	if(LCD_IF_DSI == lcdp->panel_info.lcd_if) {
		disp_sys_clk_set_rate(lcdp->dsi_clk0, clk_info_set->dsi_rate);
		disp_sys_clk_set_rate(lcdp->dsi_clk1, clk_info_set->dsi_phy_rate);
	}

	if (clk_info_temp.pll_rate != clk_info.pll_rate) {
		DE_WRN("disp %d, clk: pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld) dsi_rate_phy(%ld) clk real:pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld) dsi_rate_phy(%ld)\n",
			lcd->disp, clk_info.pll_rate, clk_info.tcon_rate, clk_info.dclk_rate, clk_info.dsi_rate, clk_info.dsi_phy_rate,
			clk_info_temp.pll_rate, clk_info_temp.tcon_rate, clk_info_temp.dclk_rate, clk_info_temp.dsi_rate, clk_info_temp.dsi_phy_rate);
	}
...
#define SUNXI_CCM_VBASE SUNXI_CCM_MOD_VBASE
	...
	if ((LCD_IF_DSI == lcdp->panel_info.lcd_if) && (0 != lcdp->irq_no_dsi)) {
		disp_sys_register_irq(lcdp->irq_no_dsi,0,disp_lcd_event_proc,(void*)lcd->disp,0,0);
...
#define SUNXI_CCM_VBASE SUNXI_CCM_MOD_VBASE
	...
	if ((LCD_IF_DSI == lcdp->panel_info.lcd_if) && (0 != lcdp->irq_no_dsi)) {
		...
		disp_sys_unregister_irq(lcdp->irq_no_dsi, disp_lcd_event_proc,(void*)lcd->disp);
	} else
#endif
	{
