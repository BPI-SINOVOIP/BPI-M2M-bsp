...
static s32 vdevice_clk_config(struct disp_device *vdevice)
{
	...
	para->lcd_dclk_freq = dclk_rate;
	disp_al_lcd_get_clk_info(vdevice->disp, &clk_info, para);
	disp_sys_clk_set_rate(vdevicep->clk_parent, clk_info.pll_rate);
	clk_info_temp.pll_rate = disp_sys_clk_get_rate(vdevicep->clk_parent);

	if (clk_info_temp.pll_rate != clk_info.pll_rate) {
		disp_al_lcd_get_clk_info(vdevice->disp, &clk_info_temp, para);
		clk_info_set = &clk_info_temp;
	}
	disp_sys_clk_set_rate(vdevicep->clk, clk_info_set->tcon_rate);
	if (clk_info_temp.pll_rate != clk_info.pll_rate)
		DE_WRN("pclk=%ld, cur=%ld\n", clk_info.dclk_rate, clk_info_temp.dclk_rate);
