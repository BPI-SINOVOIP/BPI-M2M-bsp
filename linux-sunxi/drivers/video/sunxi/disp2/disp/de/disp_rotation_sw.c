...
static int rotate_fb(disp_fb_info *src_fb, disp_rect *src_dirty,
	disp_fb_info *dst_fb, disp_rect *dst_dirty,
	void *dst_vir_addr, int deg)
{
	...
	case DISP_FORMAT_BGRA_8888:
		map_size = src_fb->size[0].width
			* src_fb->size[0].height << 2;
		Bpp[0] = 4;
		Bpp[1] = Bpp[2] = 0;
		break;
	...
	case 2:
		...
		src_offset = src_widthstep * src_dirty->y + (src_dirty->x << 1);
		dst_widthstep = dst_fb->size[0].width << 1;
		dst_offset = dst_widthstep * dst_dirty->y + (dst_dirty->x << 1);
		switch (deg) {
		case ROTATION_SW_90:
#ifdef CONFIG_DISP2_SUNXI_NEON_ROTATION
		    kernel_neon_begin();
			rot_90_u16((unsigned int *)((char *)src_vir_addr
			+ src_offset), src_widthstep, src_dirty->width,
			src_dirty->height, (unsigned int *)((char *)dst_vir_addr
			+ dst_offset), dst_widthstep);
			kernel_neon_end();
#else
			rot_90_u16_mb((unsigned short *)((char *)src_vir_addr
			+ src_offset),
			src_widthstep, src_dirty->width,
			src_dirty->height,
			(unsigned short *)((char *)dst_vir_addr
			+ dst_offset),
			dst_widthstep);
#endif
			break;
		case ROTATION_SW_270:
			rot_270_u16_mb(
			(unsigned short *)((char *)src_vir_addr
			+ src_offset),
			src_widthstep,
			src_dirty->width,
			src_dirty->height,
	...
	case 2:
		...
		case ROTATION_SW_90:
			rot_90_u16_mb((unsigned short *)((char *)src_vir_addr
			+ src_offset), src_widthstep, src_dirty->width >> dirty_fac1,
			src_dirty->height >> dirty_fac1,
			(unsigned short *)((char *)dst_vir_addr + dst_offset),
			dst_widthstep);
			break;
		case ROTATION_SW_270:
			rot_270_u16_mb((unsigned short *)((char *)src_vir_addr
			+ src_offset), src_widthstep, src_dirty->width >> dirty_fac1,
			src_dirty->height >> dirty_fac1,
	...
	disp_layer_config *lyr_config)
{
	...
	if ((0 != src->vir_addr) && (0 != dst->vir_addr)
		...
		rot_0_u32(src_addr, widthstep, src->update_rect.width,
			...
			case DISP_FORMAT_ARGB_1555:
			case DISP_FORMAT_BGRA_5551:
				rot_0_u16(src_addr, widthstep, src->update_rect.width,
				src->update_rect.height, dst_addr, widthstep);
			break;
			default:
...
static int manager_rotate_buffer(disp_fb_info *ask_fb, struct rotation_buf_info *tg_buf)
{
	...
	case DISP_FORMAT_ARGB_1555:
	case DISP_FORMAT_BGRA_5551:
		ask_total_size = (ask_fb->size[0].width * ask_fb->size[0].height) << 1;
#if defined(ONE_BUF)
		tg_buf->w_buf = &tg_buf->buf_a;
#elif defined(DOUBLE_BUF)
		if (tg_buf->w_buf == &tg_buf->buf_a) {
	...
	disp_layer_config *lyr_config, disp_rect src_dirty_rect)
{
	...
	if (DISP_FORMAT_BGRA_5551 < lyr_config->info.fb.format)
		return 0;
	...
	if (1 == lyr_config->enable) {
		update_rotate_fb(p_rot_sw_p->tg_buf.r_buf, p_rot_sw_p->tg_buf.w_buf,
			...
			&dst_dirty_rect, &rot_sw->screen_size, lyr_config);
