/*
 * drivers/video/sunxi/disp2/disp/lcd/sl006p.c
 *
...
{
	printk("[ZHOUHONGBIAO:]LCD_open_flow---------------sl006p\n");
...
{
	printk("[ZHOUHONGBIAO:]LCD_close_flow---------------sl006p\n");
...
static void LCD_power_on(u32 sel)
{
	printk("[ZHOUHONGBIAO:]LCD_power_on_start!\n");
...
//	sunxi_lcd_pin_cfg(sel, 1);
	printk("[ZHOUHONGBIAO:]LCD_power_on_end!\n");
}
...
static void LCD_bl_open(u32 sel)
{
	printk("[ZHOUHONGBIAO:]LCD_bl_open!\n");
...
{
	__u32 i;
	printk("[ZHOUHONGBIAO:]LCD_panel_init!\n");
	//__u32 hx8394a_used	= 0;//hancai
	//__u32 hx8394f_used	= 0;//longteng,jian rong hx8394a?
	//__u32 ili9881c_used	= 1;//longteng
	#if 0
	if(ili9881c_used){
		panel_rst(1);
...
//		sunxi_lcd_delay_ms(10);
		printk("[huyanhu:]hx8394f_init_setting i = %d end!\n",i);
	}else if(ili9881c_used){
#endif
		for (i=0;;i++) {
			if(ili9881c_init_setting[i].cmd == REGFLAG_END_OF_TABLE)
				...
				sunxi_lcd_dsi_dcs_write(0,ili9881c_init_setting[i].cmd,ili9881c_init_setting[i].para_list,ili9881c_init_setting[i].count);

		}
		printk("[ZHOUHONGBIAO:]ili9881c_init_setting i = %d end!\n",i);
	//}
