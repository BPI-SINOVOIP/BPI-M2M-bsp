...
static int Fb_map_boot_logo(__u32 sel, struct fb_info *info)
{
	...
	if(fb_width > image_width) {
		if(fb_height > image_height) {
			...
			screen_offset = (void *)((u32)info->screen_base + ((fb_width - image_width) / 2) * (info->var.bits_per_pixel >> 3));
		}
		...
		if(fb_height < image_height) {
			...
			image_offset = (void *)((u32)vaddr + ((image_width - fb_width) / 2) * (info->var.bits_per_pixel >> 3));
		}
	}
