				...
				const char *buf, size_t count)
{
	...
	if (strnicmp(buf, "1", 1) == 0) {
		if (1 != Hdmi_hal_get_hdcp_enable())
			Hdmi_hal_hdcp_enable(1);
	} else {
		if (0 != Hdmi_hal_get_hdcp_enable())
			Hdmi_hal_hdcp_enable(0);
	}
...
static int __devexit hdmi_resume(struct platform_device *pdev)
{
	return 0;
}
