...
static char modules_name[32] = "ac200_tv";
...
static u32  tv_clk_enable_count;
...
static int tv_init(void)
{
	...
	if(1 == ret) {
		...
		switch_dev_register(&cvbs_switch_dev);
			...
			{
				printk("<4>[TV]%s:smooth boot, type0 = %d, type1 = %d\n", __func__, output_type0, output_type1);
			...
			tv_clk_init();
			tv_clk_enable(g_tv_mode);
			tv_clk_enable_count = CLK_INIT_ENABLE_COUNT; //boot and kernel clk enable counts
		}
	} else
		tv_used = 0;
...
static int ac200_tv_probe(struct platform_device *pdev)
{
...
static int __devexit tv_remove(struct platform_device *pdev)
{
	if(tv_device)
...
static struct platform_driver ac200_tv_driver = {
	.driver = {
		.name = "tv",
		.owner = THIS_MODULE,
	},
	.probe = ac200_tv_probe,
	.remove = __devexit_p(tv_remove),
	.shutdown = tv_shutdown,
};


module_platform_driver(ac200_tv_driver);

MODULE_AUTHOR("zengqi");
MODULE_DESCRIPTION("ac200_tv driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:ac200tv");
