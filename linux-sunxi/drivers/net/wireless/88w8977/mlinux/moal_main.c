...
#ifdef CUSTOMER_HW_ALLWINNER
static int woal_get_sdc_id(void)
{
...
static int woal_wlan_set_carddetect(bool present)
{
	int err = 0;
...
static int woal_wlan_set_power(bool on_off)
{
...
#ifdef CUSTOMER_HW_ALLWINNER
	if((ret = woal_get_sdc_id()) != 0){
		printk("%s: get sdc id failed!\n", __func__);
		return -1;
	}

	woal_wlan_set_power(1);

	woal_wlan_set_carddetect(1);
#endif
...
#ifdef CUSTOMER_HW_ALLWINNER
	woal_wlan_set_carddetect(0);
	woal_wlan_set_power(0);
#endif
}
