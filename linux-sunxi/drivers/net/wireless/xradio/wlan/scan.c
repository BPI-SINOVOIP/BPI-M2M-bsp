		   ...
		   struct cfg80211_scan_request *req)
{
	...
	if (hw_priv->connect_state[0] > XRADIO_CONNECT_DELAYED && req->n_ssids == 0 && priv->if_id == 0) {
		unsigned long timedelay = hw_priv->connect_time[0] + SCAN_MAX_DELAY;
		if (time_before(jiffies, timedelay)) {
			scan_printk(XRADIO_DBG_NIY, "vif0 connect state=%d, scan delay %ldms\n",
				hw_priv->connect_state[0], (timedelay - jiffies)*1000/HZ);
			return -EBUSY;
		}
		hw_priv->connect_state[0] = XRADIO_CONNECT_DELAYED;
	}
