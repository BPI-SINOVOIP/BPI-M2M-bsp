...
#define WIFI_CONF_PATH    "/netPrivate/xr_wifi.conf"
...
enum xradio_connect_status {
	XRADIO_CONNECT_NONE  = 0,
	XRADIO_CONNECT_DELAYED,
	XRADIO_CONNECT_AUTH,
	XRADIO_CONNECT_ASSOC,
	XRADIO_CONNECT_8021X,
	XRADIO_CONNECT_DHCP
};
...
struct xradio_common {
	...
	* after the mechanism is clear */
	...
	enum xradio_connect_status connect_state[XRWL_MAX_VIFS-1];
	unsigned long       connect_time[XRWL_MAX_VIFS-1];
	atomic_t            suspend_state;
#ifdef SUPPORT_ACS
	wait_queue_head_t	ms_wq;
	atomic_t			ms_status;
	struct survey_info	survey;
	struct survey_info	debug_survey[13];
