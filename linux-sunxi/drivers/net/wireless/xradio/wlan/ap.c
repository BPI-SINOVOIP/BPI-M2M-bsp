			     ...
			     struct ieee80211_bss_conf *info, u32 changed)
{
	...
	if (changed &
		...
		if (info->assoc) {	/* TODO: ibss_joined */
			...
			if (sta) {
				...
				ap_printk(XRADIO_DBG_WARN, "BSS_CHANGED_ASSOC no sta.\n");
	...
	if (changed & (BSS_CHANGED_ASSOC | BSS_CHANGED_ERP_CTS_PROT)) {
		...
		ap_printk(XRADIO_DBG_WARN, "BSS_CHANGED_ERP_CTS_PROT use_cts_prot=%d.\n",
				info->use_cts_prot);
	}
#endif
					      ...
					      u16  Duration)
{
	...
	status = wait_event_interruptible_timeout(hw_priv->ms_wq, atomic_read(&hw_priv->ms_status), waiting_time);
	if (!status) {
...
int xradio_get_survey(struct ieee80211_hw *hw, int idx, struct survey_info *survey)
{
	struct xradio_common *hw_priv = hw->priv;
	...
	ret = xradio_do_chan_mearsure(hw_priv, measure_type, channelnum, 80);
	if (ret)
		return -ENOENT;
