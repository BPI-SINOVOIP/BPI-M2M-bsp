...
#define HIGH_RATE_MAX_RETRY  7 /* 9 */
#else
	...
	struct ieee80211_tx_rate *rates, size_t count)
{
	...
	if (WSM_CAPS_11N_TO_11BG(hw_priv->wsm_caps) && !priv->vif->p2p &&
		/* ((priv->oper_rates | priv->base_rates) & BIT(2)) && */
...
void xradio_tx(struct ieee80211_hw *dev, struct sk_buff *skb)
{
	...
	if (ieee80211_is_auth(frame->frame_control)) {
		hw_priv->connect_time[priv->if_id] = jiffies;
		hw_priv->connect_state[priv->if_id] = XRADIO_CONNECT_AUTH;
		...
		if (t.txpriv.use_bg_rate) {
			hw_priv->connect_time[priv->if_id] = jiffies;
			hw_priv->connect_state[priv->if_id] = XRADIO_CONNECT_DHCP;
		}
	} else if (ieee80211_is_deauth(frame->frame_control) ||
		   ieee80211_is_disassoc(frame->frame_control)) {
		hw_priv->connect_state[priv->if_id] = XRADIO_CONNECT_NONE;
	}
