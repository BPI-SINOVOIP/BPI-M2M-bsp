...
int xradio_register_bh(struct xradio_common *hw_priv)
{
	...
	if (IS_ERR(hw_priv->bh_thread)) {
		...
		int ret = sched_setscheduler(hw_priv->bh_thread, SCHED_NORMAL, &param);
		if (ret)
...
static int xradio_bh(void *arg)
{
	...
	for (;;) {
		...
		if (!status) {
			...
			xradio_bh_read_ctrl_reg(hw_priv, &ctrl_reg);
			if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
				bh_printk(XRADIO_DBG_MSG, "miss interrupt!\n");
...
tx:
		...
		if (tx && tx_allowed) {
			...
			if (ret <= 0) {
				...
				if (tx_burst > 1 || tx_bursted < tx_burst_limit) {
					...
					++tx_bursted;
					goto tx;
				} else if ((ctrl_reg & HIF_CTRL_NEXT_LEN_MASK)) {
		...
		/*One more to check if there are frames to be rx or tx. */
		if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
			DBG_INT_ADD(next_rx_cnt);
			rx = 1;
			goto data_proc;
		} else if (tx && hw_priv->hw_bufs_used <
			hw_priv->wsm_caps.numInpChBufs - 1) {
			tx = 1;
			goto data_proc;
		} else {
			/*we look to rx if no tx.*/
			xradio_bh_read_ctrl_reg(hw_priv, &ctrl_reg);
			if (ctrl_reg & HIF_CTRL_NEXT_LEN_MASK) {
				DBG_INT_ADD(fix_miss_cnt);
				rx = 1;
