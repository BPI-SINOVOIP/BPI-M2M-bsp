	...
	*/
	...
	if (measure_cmpl.Status == WSM_STATUS_SUCCESS) {
		...
		case ChannelLoadMeasurement:
...
#endif
#ifdef SUPPORT_ACS
			sband = hw_priv->hw->wiphy->bands[IEEE80211_BAND_2GHZ];
			...
			hw_priv->survey.filled = SURVEY_INFO_CHANNEL_TIME | SURVEY_INFO_CHANNEL_TIME_BUSY | SURVEY_INFO_NOISE_DBM;

			/* fill channel survey debug messages */
			if (chan->hw_value < 14) {
				mutex_lock(&hw_priv->ms_mutex);
				memcpy(&(hw_priv->debug_survey[chan->hw_value - 1]), &(hw_priv->survey), sizeof(struct survey_info));
	...
	if (dbg_id == 5) {
		...
		if (dbg_buf_len >= 4) {
			...
			wsm_printk(XRADIO_DBG_ALWY,
				   "[FW-DEBUG] DbgId = %d, data = %d", dbg_id, buf_data);
		} else {
