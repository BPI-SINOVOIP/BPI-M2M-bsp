...
int
dhd_conf_intvar_get_reg(dhd_pub_t *dhd, int cmd, char *name, uint reg, int *retval)
{
	union {
	...
	if (bcm_mkiovar(name, (char *)(&reg), sizeof(reg), (char *)(&var), sizeof(var.buf))) {
		ret = dhd_wl_ioctl_cmd(dhd, cmd, (void*)&var, sizeof(var.buf), FALSE, 0);
		if (!ret) {
...
void
dhd_conf_set_btc_mgmt(dhd_pub_t *dhd)
{
	char flag[8] = { 0, 00, 00, 00, 0x1, 0x0, 0x00, 0x00 };
	int i, mpc = 1, first = 1;
	uint *flag_tmp = (uint *)&flag[4];

	for (i=0; i<sizeof(dhd->conf->btc_flags_mgmt.bit)/sizeof(dhd->conf->btc_flags_mgmt.bit[0]); i++) {
		if (dhd->conf->btc_flags_mgmt.bit[i] >= 0) {
			flag[0] = dhd->conf->btc_flags_mgmt.bit[i];
			*flag_tmp = dhd->conf->btc_flags_mgmt.value[i];
			mpc = 0;
			if (first) {
				first = 0;
				dhd_conf_set_intiovar(dhd, WLC_SET_VAR, "mpc", mpc, 0, FALSE);
			}
			printf("%s: btc_flags %d 0x%x\n", __FUNCTION__, flag[0], *flag_tmp);
			dhd_conf_set_bufiovar(dhd, WLC_SET_VAR, "btc_flags", (char *)flag, sizeof(flag), FALSE);
		}		
	}
	if (!first) {
		mpc = 1;
	...
	for (i=0; i<sizeof(dhd->conf->btc_params_mgmt.bit)/sizeof(dhd->conf->btc_params_mgmt.bit[0]); i++) {
		if (dhd->conf->btc_params_mgmt.bit[i] >= 0) {
			flag[0] = dhd->conf->btc_params_mgmt.bit[i];
			*flag_tmp = dhd->conf->btc_params_mgmt.value[i];
			printf("%s: btc_params %d 0x%x\n", __FUNCTION__, flag[0], *flag_tmp);
			dhd_conf_set_bufiovar(dhd, WLC_SET_VAR, "btc_params", (char *)flag, sizeof(flag), FALSE);
		}		
	}
...
bool
dhd_conf_read_btc_flags(dhd_pub_t *dhd, char *full_param, uint len_param)
{
	int i;
	char *pch, *pick_tmp;
	struct dhd_conf *conf = dhd->conf;
	...
	/* Process btc_flags_mgmt and btc_params_mgmt:
	 * btc_flags_mgmt=[bit#1]/[val#1], [bit#2]/[val#2]
	 * Ex: btc_flags_mgmt=1/1, 4/0
	 * btc_params_mgmt=[bit#1]/[val#1], [bit#2]/[val#2]
	 * Ex: btc_params_mgmt=1/1, 4/0
	 */
	if (!strncmp("btc_flags_mgmt=", full_param, len_param)) {
		pick_tmp = data;
		for (i=0; i<7; i++) {
			/* Process country code */
			pch = bcmstrtok(&pick_tmp, "/", 0);
			if (!pch)
				break;
			conf->btc_flags_mgmt.bit[i] = (int)simple_strtol(pch, NULL, 0);
			pch = bcmstrtok(&pick_tmp, ", ", 0);
			if (!pch)
				break;
			conf->btc_flags_mgmt.value[i] = (int)simple_strtol(pch, NULL, 0);
			CONFIG_ERROR(("%s: btc_flags %d %d\n", __FUNCTION__,
				conf->btc_flags_mgmt.bit[i],
				conf->btc_flags_mgmt.value[i]));
		}
	}
	else if (!strncmp("btc_params_mgmt=", full_param, len_param)) {
		pick_tmp = data;
		for (i=0; i<7; i++) {
			/* Process country code */
			pch = bcmstrtok(&pick_tmp, "/", 0);
			if (!pch)
				break;
			conf->btc_params_mgmt.bit[i] = (int)simple_strtol(pch, NULL, 0);
			pch = bcmstrtok(&pick_tmp, ", ", 0);
			if (!pch)
				break;
			conf->btc_params_mgmt.value[i] = (int)simple_strtol(pch, NULL, 0);
			CONFIG_ERROR(("%s: btc_params %d 0x%x\n", __FUNCTION__,
				conf->btc_params_mgmt.bit[i],
				conf->btc_params_mgmt.value[i]));
		}
	}
	else
		return false;
	...
	if (len > 0 && len < MAXSZ_CONFIG) {
		...
		while (start_pos < len) {
			...
			if (dhd_conf_read_log_level(dhd, pick, len_param))
				...
				continue;
			else if (dhd_conf_read_btc_flags(dhd, pick, len_param))
				continue;
#ifdef PKT_FILTER_SUPPORT
...
#endif
	memset(&conf->btc_flags_mgmt, -1, sizeof(wl_btc_flags_t));
	memset(&conf->btc_params_mgmt, -1, sizeof(wl_btc_params_t));
