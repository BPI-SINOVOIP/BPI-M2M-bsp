...
static int __init sunxi_rtc_probe(struct platform_device *pdev)
{
	...
	int err = 0;
#ifdef SUPPORT_SHUTDOWN_ALARM
       int reg_val = 0, alarm_int_en, alarm_int_stat;
#endif
	pr_debug("%s,line:%d\n", __func__, __LINE__);
	...
	mutex_init(&rtc_dev->mutex);
        ...
        * */
       alarm_int_stat = rtc_read_reg(rtc_dev->ac100, ALM_INT_STA_REG_OFF);
       alarm_int_en = rtc_read_reg(rtc_dev->ac100, RTC_GP_REG_N_OFF);
       if ((alarm_int_stat & 0x1) && (alarm_int_en & 0x1))
               alarm_in_booting = 1;
#endif
