...
#ifdef SUPPORT_SHUTDOWN_ALARM
int alarm_in_booting = 0;
...
static int __devinit sunxi_rtc_probe(struct platform_device *pdev)
{
...
#ifdef SUPPORT_SHUTDOWN_ALARM
       unsigned long alarm_cnt, alarm_cur, alarm_en, alarm_int_ctrl, alarm_int_stat;
#endif
	...
	if (sunxi_rtc_alarmno <= 0) {
       ...
       alarm_cnt = sunxi_rtc_read(SUNXI_RTC_ALARM_COUNTER_REG);
       alarm_cur = sunxi_rtc_read(SUNXI_RTC_ALARM_CURRENT_REG);
       alarm_en = sunxi_rtc_read(SUNXI_ALARM_EN_REG);
       alarm_int_ctrl = sunxi_rtc_read(SUNXI_ALARM_INT_CTRL_REG);
       alarm_int_stat = sunxi_rtc_read(SUNXI_ALARM_INT_STATUS_REG);
