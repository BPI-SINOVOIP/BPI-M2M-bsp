...
#if defined CONFIG_ARCH_SUN8IW8 || defined CONFIG_ARCH_SUN8IW5
static int partitions_register(struct mtd_info *mtd, \
		struct mtd_part_parser_data *ppdata)
{
	...
	partitions = kzalloc(sizeof(struct mtd_partition)*(sunxi_mbr->PartCount+1), GFP_KERNEL);
	if (partitions == NULL) {
    ...
    partitions[0].size = sunxi_mbr->array[0].addrlo*NOR_BLK_SIZE + MBR_OFFSET;
	M25_DBG("NorFlash partition %d: name=%s, offset=0x%llx, size=0x%llx\n", i,
			partitions[0].name, partitions[0].offset, partitions[0].size);
	...
	for (i=0; i<sunxi_mbr->PartCount; i++) {
        ...
        if (!strcmp(partitions[i+1].name, "UDISK")) {
            partitions[i+1].size = flash->mtd.size - size;
        } else {
		    partitions[i+1].size   = sunxi_mbr->array[i].lenlo*NOR_BLK_SIZE;
            size += partitions[i+1].size;
            if (i == 0)
                size += partitions[i+1].offset;
        }
		M25_DBG("NorFlash partition %d: name=%s, offset=0x%llx, size=0x%llx\n", i+1,
...
/*
 *spinor with 32M size should used 4-bytes-addr mode
 *BROM just support 3-bytes-addr mode
 *so,BROM will read boot0 wrongly
 *we should exit 4-bytes-addr mode when reboot
 */
