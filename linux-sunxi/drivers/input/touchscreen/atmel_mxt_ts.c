...
#define MXT_BOOTLOADER_WAIT	0x36E5	/* 1 minute */
...
static const unsigned short normal_i2c[2] = {0x4a, I2C_CLIENT_END};
static __u32 twi_id;
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	strlcpy(info->type, config_info.name, I2C_NAME_SIZE);

	pdata->gpio_reset = config_info.wakeup_gpio.gpio;
	pdata->gpio_int = config_info.irq_gpio.gpio;
	pr_info("gpio_int: gpio=%d,mul_sel=%d,pull=%d,dlevel=%d,data=%d\n",
		config_info.irq_gpio.gpio, config_info.irq_gpio.mul_sel,
		config_info.irq_gpio.pull, config_info.irq_gpio.drv_level,
		config_info.irq_gpio.data);

	if (config_info.irq_gpio.pull != GPIO_PULL_DEFAULT) {
		sunxi_gpio_to_name(pdata->gpio_int, pin_name);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_PUD,
						config_info.irq_gpio.pull);
...
static int __init mxt_ts_init(void)
{
	int ret = -1;
