...
static unsigned char keypad_mapindex[64] = {
	...
	3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3,	/* key 3, 21-40 */
	4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,					/* key 4, 41-60 */
	5, 5, 														/* key 5, */
};
#endif
...
#define dprintk(level_mask, fmt, arg...)	do {if (debug_mask & level_mask)  printk(fmt , ## arg); } while (0)
...
static void sunxi_keyboard_ctrl_set(enum key_mode key_mode, u32 para)
{
	...
	if (0 != para)
		ctrl_reg = sunxi_smc_readl((void __iomem *)(KEY_BASSADDRESS + LRADC_CTRL));
	...
	sunxi_smc_writel(ctrl_reg, (void __iomem *)(KEY_BASSADDRESS + LRADC_CTRL));
}
...
static void sunxi_keyboard_int_set(enum int_mode int_mode, u32 para)
{
	...
	if (0 != para)
		ctrl_reg = sunxi_smc_readl((void __iomem *)(KEY_BASSADDRESS + LRADC_INTC));
	...
	sunxi_smc_writel(ctrl_reg, (void __iomem *)(KEY_BASSADDRESS + LRADC_INTC));
}

static u32 sunxi_keyboard_read_ints(void)
{
	u32 reg_val;
	reg_val  = sunxi_smc_readl((void __iomem *)(KEY_BASSADDRESS + LRADC_INT_STA));
...
{
	sunxi_smc_writel(reg_val, (void __iomem *)(KEY_BASSADDRESS + LRADC_INT_STA));
}

static u32 sunxi_keyboard_read_data(unsigned long addr)
{
	u32 reg_val;
	reg_val = sunxi_smc_readl((void __iomem *)(addr));
...
static irqreturn_t sunxi_isr_key(int irq, void *dummy)
{
	...
	/*sunxi_smc_writel(reg_val,KEY_BASSADDRESS + LRADC_INT_STA);*/
	...
	if (reg_val & LRADC_ADC0_DATAPEND) {
		...
		if (key_val < 0x3f) {
			...
			compare_buffer[key_cnt] = keypad_mapindex[key_val&0x3f];
		}
