 ...
 *
 */
static void __USBC_ConfigFifo_TxEp(__u32 usbc_base_addr, __u32 is_double_fifo, __u32 fifo_size, __u32 fifo_addr)
{
	__u32 temp = 0;
...
static void __USBC_ConfigFifo_RxEp_Default(__u32 usbc_base_addr)
{
 ...
 *
 */
static void __USBC_ConfigFifo_RxEp(__u32 usbc_base_addr, __u32 is_double_fifo, __u32 fifo_size, __u32 fifo_addr)
{
	__u32 temp = 0;
	__u32 size = 0;   //fifo_size = 2 ^ (size + 3)
	__u32 addr = 0;   //fifo_addr = addr * 8
...
__hdle USBC_open_otg(__u32 otg_no)
{
	...
	//--<1>--otg_no cannot exceed the num supported.
	if (otg_no >= USBC_MAX_CTL_NUM) {
		return 0;
	}

	//--<2>--find a unsed one, max support 8 times synchronously
...
__s32 USBC_init(bsp_usbc_t *usbc)
{
	...
	/* save the usb controller's base addr */
	/*
