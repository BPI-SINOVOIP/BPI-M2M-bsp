...
static inline int bluesleep_can_sleep(void)
{
	/* check if HOST_WAKE_BT_GPIO and BT_WAKE_HOST_GPIO are both deasserted */
	  return (gpio_get_value(bsi->ext_wake) != bsi->bt_wake_assert) &&
...
static void bluesleep_sleep_work(struct work_struct *work)
{
	if (bluesleep_can_sleep()) {
	  ...
	  } else if((gpio_get_value(bsi->ext_wake) != bsi->bt_wake_assert) && !test_bit(BT_ASLEEP, &flags) ) {
		gpio_set_value(bsi->ext_wake, bsi->bt_wake_assert);
		mod_timer(&tx_timer, jiffies + (TX_TIMER_INTERVAL * HZ));
	}else {
...
static void bluesleep_outgoing_data(void)
{
	...
	if (gpio_get_value(bsi->ext_wake) != bsi->bt_wake_assert) {
...
{
	*eof = 1;
	return sprintf(page, "btwake:%u\n", (gpio_get_value(bsi->ext_wake) == bsi->bt_wake_assert));
}
...
static int __init bluesleep_probe(struct platform_device *pdev)
{
	...
	bsi = devm_kzalloc(dev,sizeof(struct bluesleep_info), GFP_KERNEL);
	if (!bsi)
		return -ENOMEM;
	...
	type = script_get_item("bt_para", "bt_wake_invert", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		BT_INFO("has no bt_wake_invert\n");
	} else {
		bsi->bt_wake_assert = !val.val;
	}
	...
	ret = gpio_direction_output(bsi->ext_wake, bsi->bt_wake_assert);
	if (ret < 0) {
