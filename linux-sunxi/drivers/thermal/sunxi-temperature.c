...
#elif defined(CONFIG_ARCH_SUN8IW7P1) || defined(CONFIG_ARCH_SUN8IW9P1)
...
#if defined(CONFIG_ARCH_SUN9IW1P1) || defined(CONFIG_ARCH_SUN8IW6P1) || defined(CONFIG_ARCH_SUN8IW7P1) || defined(CONFIG_ARCH_SUN8IW9P1)
		avg_temp[i] *= 1000;
		if (0 != avg_temp[i])
...
static void sunxi_ths_work_func(struct work_struct *work)
{
	...
	if ((1000 < data->temp_data1[data->circle_num]) && (1000 < data->temp_data2[data->circle_num])
		...
		data->circle_num++;
	calc_temperature(3, REG_DIV, REG_MIN, data);
...
static void sunxi_ths_clk_cfg(void)
{
	...
	if (clk_set_rate(ths_clk, 24000000)) {
		printk(KERN_DEBUG "set ths clock freq to 24M failed!\n");
	}
...
static void sunxi_ths_reg_init(void)
{
	...
	reg_value = temperature_to_reg(ths_zone_2.sunxi_ths_sensor_conf->trip_data->trip_val[0], TEM_MUL, TEM_MIN);
