...
static int sunxi_pinctrl_test_remove(struct platform_device *pdev)
{
	struct sunxi_pinctrl_test_class *pclass = NULL;

	dev_set_name(&pdev->dev, "sunxi_pinctrl_test");
	printk("%s() - dev_name(dev): %s\n", __func__, dev_name(&pdev->dev));

	pclass = platform_get_drvdata(pdev);
	device_destroy(sunxi_pinctrl_test_init_class, pclass->dev->devt);
	class_destroy(sunxi_pinctrl_test_init_class);
	return 0;
}

static void sunxi_pinctrl_test_release(struct device *dev)
{
}

static struct platform_driver sunxi_pinctrl_test_driver={
	.probe		= sunxi_pinctrl_test_probe,
	.remove		= sunxi_pinctrl_test_remove,
	.driver		={
...
static struct platform_device sunxi_pinctrl_test_devices={
	...
	.dev.release = sunxi_pinctrl_test_release,
};
