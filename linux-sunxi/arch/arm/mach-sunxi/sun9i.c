	...
	if((IS_ENABLED(CONFIG_CMA) && is_cma) || ((!IS_ENABLED(CONFIG_CMA) || force) && !is_cma))
		for(i=0;i<ion_init_max;i++) {
			if(ion_reserve[i]) {
				...
				/* Ensure that 32bit device can access it, such as lcd, DMA etc. */
				ion_mem.start = (unsigned long)(min((mem_start + mem_size),
						(phys_addr_t)(0xFFFFFFFFUL)) - ion_mem.size);
			} else {
				early_printk("%s: ion reserve: [0x%x, 0x%x]!\n", __func__, 
						(int)ion_mem.start, (int)(ion_mem.start + ion_mem.size));
				break;
			}
			       ...
			       struct meminfo *meminfo)
{
	...
	for (t = tags; t->hdr.size; t = tag_next(t)) {
		if (t->hdr.tag == ATAG_MEM && t->u.mem.size) {
			/* Avoid parsing it at parse_tags to add memory duplication */
			t->hdr.tag = ATAG_NONE;
			mem_start = t->u.mem.start;
			mem_size = t->u.mem.size;

			/* Fixup memory up to 8G */
			if(mem_size  && (mem_size <=16384) && mem_start) {
#ifdef CONFIG_ARM_LPAE            
				mem_size = ((phys_addr_t)t->u.mem.size*1024*1024);
#else
				mem_size = (t->u.mem.size >=4096) 
						? (((phys_addr_t)~0UL - PLAT_PHYS_OFFSET))
						: ((phys_addr_t)t->u.mem.size*1024*1024);
#endif                
				t->u.mem.size=0;
			}
...
#ifdef CONFIG_DRAM_TRAINING_RESERVE_MEM
	dramfreq_mem_size = mem_size;
	dramfreq_mem_size_mb = dramfreq_mem_size >> 20;
#endif
	...
	early_printk("nr_banks: %d, bank.start: 0x%llx, bank.size: 0x%llx\n",
			meminfo->nr_banks, (u64)meminfo->bank[0].start,
			(u64)meminfo->bank[0].size);
}
