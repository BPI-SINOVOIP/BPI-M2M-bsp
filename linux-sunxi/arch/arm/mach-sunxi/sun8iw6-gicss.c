...
static void gic_mask_irq(struct irq_data *d)
{
	...
	cpu_gic_writel(mask, gic_dist_base(d) + GIC_DIST_ENABLE_CLEAR + (gic_irq(d) / 32) * 4);
...
static void gic_unmask_irq(struct irq_data *d)
{
	...
	cpu_gic_writel(mask, gic_dist_base(d) + GIC_DIST_ENABLE_SET + (gic_irq(d) / 32) * 4);
	raw_spin_unlock(&irq_controller_lock);
}
...
static void gic_eoi_irq(struct irq_data *d)
{
	...
	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(gic_irq(d), gic_cpu_base(d) + GIC_CPU_AEOI);
	} else {
...
static int gic_set_type(struct irq_data *d, unsigned int type)
{
	...
	val = cpu_gic_readl(base + GIC_DIST_CONFIG + confoff);
	 ...
	 * the configuration
	 */
	if (cpu_gic_readl(base + GIC_DIST_ENABLE_SET + enableoff) & enablemask) {
		cpu_gic_writel(enablemask, base + GIC_DIST_ENABLE_CLEAR + enableoff);
		enabled = true;
	}
	...
	if (enabled)
		cpu_gic_writel(enablemask, base + GIC_DIST_ENABLE_SET + enableoff);
...
void gic_cpu_exit(unsigned int gic_nr)
{
	...
	cpu_gic_writel(0, base + GIC_CPU_CTRL);
}
...
asmlinkage void __exception_irq_entry gic_handle_irq(struct pt_regs *regs)
{
	...
	do {
		if (sunxi_soc_is_secure()) {
			irqstat = cpu_gic_readl(cpu_base + GIC_CPU_AINTACK);
		} else {
		...
		if (irqnr < 16) {
			if (sunxi_soc_is_secure()) {
				cpu_gic_writel(irqstat, cpu_base + GIC_CPU_AEOI);
			} else {
...
static void gic_handle_cascade_irq(unsigned int irq, struct irq_desc *desc)
{
	...
	raw_spin_lock(&irq_controller_lock);
	if (sunxi_soc_is_secure()) {
		status = cpu_gic_readl(gic_data_cpu_base(chip_data) + GIC_CPU_AINTACK);
	} else {
...
static void __init gic_dist_init(struct gic_chip_data *gic)
{
	...
	cpu_gic_writel(0, base + GIC_DIST_CTRL);

	/*
	 * Set all global interrupts to be level triggered, active low.
	 */
	for (i = 32; i < gic_irqs; i += 16)
		cpu_gic_writel(0, base + GIC_DIST_CONFIG + i * 4 / 16);

	/*
	 * Set all global interrupts to this CPU only.
	 */
	cpumask = cpu_gic_readl(base + GIC_DIST_TARGET + 0);
	 ...
	 */
	for (i = 32; i < gic_irqs; i += 4)
		cpu_gic_writel(0xa0a0a0a0, base + GIC_DIST_PRI + i * 4 / 4);
	...
	for (i = 32; i < gic_irqs; i += 32)
		cpu_gic_writel(0xffffffff, base + GIC_DIST_ENABLE_CLEAR + i * 4 / 32);

	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(0x3, base + GIC_DIST_CTRL);
	} else {
		cpu_gic_writel(0x1, base + GIC_DIST_CTRL);
	}
}
...
static void __cpuinit gic_cpu_init(struct gic_chip_data *gic)
{
	...
	cpu_mask = cpu_gic_readl(dist_base + GIC_DIST_TARGET + 0);
	 ...
	 */
	cpu_gic_writel(0xffff0000, dist_base + GIC_DIST_ENABLE_CLEAR);
	cpu_gic_writel(0x0000ffff, dist_base + GIC_DIST_ENABLE_SET);

	/*
	 * Set priority on PPI and SGI interrupts
	 */
	for (i = 0; i < 32; i += 4)
		cpu_gic_writel(0xa0a0a0a0, dist_base + GIC_DIST_PRI + i * 4 / 4);

	cpu_gic_writel(0xf0, base + GIC_CPU_PRIMASK);

	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(0xb, base + GIC_CPU_CTRL);
	} else {
		cpu_gic_writel(0x1, base + GIC_CPU_CTRL);
	}
}
...
static void gic_dist_restore(unsigned int gic_nr)
{
	...
	cpu_gic_writel(0, dist_base + GIC_DIST_CTRL);
	...
	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(0x3, dist_base + GIC_DIST_CTRL);
	} else {
		cpu_gic_writel(0x1, dist_base + GIC_DIST_CTRL);
	}
}
...
static void gic_cpu_restore(unsigned int gic_nr)
{
	...
	for (i = 0; i < DIV_ROUND_UP(32, 4); i++)
		cpu_gic_writel(0xa0a0a0a0, dist_base + GIC_DIST_PRI + i * 4);

	cpu_gic_writel(0xf0, cpu_base + GIC_CPU_PRIMASK);
	
	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(0xb, cpu_base + GIC_CPU_CTRL);
	} else {
		cpu_gic_writel(0x1, cpu_base + GIC_CPU_CTRL);
	}
}
...
void gic_raise_softirq(const struct cpumask *mask, unsigned int irq)
{
	...
	if (sunxi_soc_is_secure()) {
		cpu_gic_writel(((map << 16) | (1 << 15) | irq), gic_data_dist_base(&gic_data[0]) + GIC_DIST_SOFTINT);
	} else {
...
bool gic_pending_irq(unsigned int gic_nr)
{
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
		...
		pr = cpu_gic_readl(dist_base + GIC_DIST_PENDING_SET + i * 4);
		er = cpu_gic_readl(dist_base + GIC_DIST_ENABLE_SET + i * 4);
