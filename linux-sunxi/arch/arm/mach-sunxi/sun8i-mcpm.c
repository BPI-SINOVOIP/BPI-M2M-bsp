 ...
 */
static int sun8i_cpu_use_count[MAX_NR_CLUSTERS][MAX_CPUS_PER_CLUSTER] = {{0}};
static int sun8i_cluster_use_count[MAX_NR_CLUSTERS] = {0};
...
int sun8i_cpu_power_set(unsigned int cluster, unsigned int cpu, bool enable)
{
	...
	if (enable) {
		...
		cpumask_set_cpu(((cluster)*CORES_PER_CLUSTER + cpu), &cpu_power_up_state_mask);
		...
		if (cpu == 0) {
			if (soc_version == SUN8IW6P1_REV_A) {
				...
				cpumask_clear_cpu(((cluster)*CORES_PER_CLUSTER + cpu), &cpu_power_up_state_mask);
				return 0;
			} else {
				/* bit4: C1_cpu0 */
				cpu = 4;
...
static int sun8i_mcpm_power_up(unsigned int cpu, unsigned int cluster)
{
	...
	if (sun8i_cpu_use_count[cluster][cpu] == 1) {
		 ...
		 */
		pr_err("sun8i_cpu_use_count[%d][%d] is invalid: %d\n", cluster, cpu,
			sun8i_cpu_use_count[cluster][cpu]);
		BUG();
	} else {
                                   ...
                                   unsigned long action, void *parg)
{
	...
	pr_debug("%s() - cluster: %d, action = %ld\n", __func__, cluster, action);
	switch (action) {
		...
		/* First, power-off the other cluster which has no boot cpu. */
