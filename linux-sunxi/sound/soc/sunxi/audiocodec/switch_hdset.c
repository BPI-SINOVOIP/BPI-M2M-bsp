 ...
 * allwinnertech Co., Ltd. <www.allwinnertech.com>
	...
	if (!headphone_direct_used) {
		if (((tmp >= 0xb) && (switch_data->mode != FOUR_HEADPHONE_PLUGIN) && (switch_data->state != 2) && (switch_data->reset_flag == 0))&&(switch_data->mode!=HOOK_CHCCK_DOWN)) {
			...
			if ((switch_data->check_three_count > CIRCLE_COUNT) && (switch_data->state != 2)) {
				...
				input_report_key(switch_data->key, KEY_HP, 1);
			...
			if (switch_data->check_plugout_count > CIRCLE_COUNT) {
				...
				input_report_key(switch_data->key, KEY_HP, 0);
				input_sync(switch_data->key);
				}
				SWITCH_DBG("%s,line:%d,switch_data->state:%d,switch_data->reset_flag:%d, tmp:%x\n", __func__, __LINE__, switch_data->state, switch_data->reset_flag, tmp);
				switch_data->check_plugout_count = 0;
		...
		if (tmp > 1) {
			...
			if (switch_data->check_plugout_count > CIRCLE_COUNT) {
				/*
				* it means the three sections
				* earphone has plun out
				*/
				switch_data->state 		= 0;
				schedule_work(&switch_data->work);
				SWITCH_DBG("%s,line:%d\n", __func__, __LINE__);
...
static irqreturn_t audio_hmic_irq(int irq, void *dev_id)
{
	...
	/*
	*	while headset(four section headphone) plug out,
	*	it will detect headphone(three section headphone) first.
	*	while headset plug out,
	*	maybe it will detect headset had plug out,
	*	however it still has three section at headset jack,
	*	at this moment, it should redetect headset plug in/out,
	*	to forbit hook key misjudge;
	*/
			...
			*/
			...
			if ((debounce_val[0]==debounce_val[1])&&(debounce_val[1]==debounce_val[2])&&(debounce_val[2]==debounce_val[3])) {
				...
				if (debounce_ave >= 1) {
					SWITCH_DBG("h-5\n");
					/*
					* disable hardware data irq,
					* using software timer
					* to judge it is headphone
					* or headset(four section headphone).
					*/
					hmic_wr_control(SUNXI_HMIC_CTL,\
						0x1, HMIC_DIRQ, 0x0);
					if (((&switch_data->timer) != NULL)) {
						del_timer(&switch_data->timer);
					}
				...
				/*
				* it has 0 data,
				* judge whether headset/headphone has plug out
				*/
				if ((debounce_val[0]==0)||(debounce_val[1]==0)||(debounce_val[2]==0)||(debounce_val[3]==0)) {
					...
					/*
					* ADC data is unstable,
					* using software timer debounce again
					*/
					hmic_wr_control(SUNXI_HMIC_DATA\
					, 0x1, HMIC_DIRQ, 0x1);
				}
			}
		} else {
			SWITCH_DBG("h-7\n");
			/*
			*	using software timer judge
			*	whether headphone/headset has plug out.
			*/
			if (((&switch_data->timer) != NULL)) {
				del_timer(&switch_data->timer);
			}
...
static int __devinit gpio_switch_probe(struct platform_device *pdev)
{
	...
	set_bit(KEY_HP, switch_data->key->keybit);
...
static int switch_suspend(struct platform_device *pdev,pm_message_t state)
{
	...
	/*
	*Need more than 450ms to ensure the pa can close normal after set HPPAEN to 0.
	*Maybe cause abnormal power consumption without enough delay.so msleep 500 currently.
	*/
	msleep(500);
