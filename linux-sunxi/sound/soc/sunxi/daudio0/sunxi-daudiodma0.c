...
static dma_addr_t daudio0_dma_addr = 0;
static unsigned char *daudio0_dma_area;	/* DMA area */
#endif
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		} else if (SNDRV_PCM_FORMAT_S24_LE == params_format(params)) {
			slave_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
			slave_config.src_addr_width = DMA_SLAVE_BUSWIDTH_4_BYTES;
		} else {
			pr_info("no support format\n");
		}
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK && BOOT_MUSIC_PARA != 1) {
		daudio0_dma_addr = substream->dma_buffer.addr;
		daudio0_dma_area = substream->dma_buffer.area;
...
static int sunxi_pcm_hw_free(struct snd_pcm_substream *substream)
{
	#if defined CONFIG_ARCH_SUN8IW6 || defined CONFIG_ARCH_SUN9IW1
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		substream->dma_buffer.addr = daudio0_dma_addr;
		substream->dma_buffer.area = daudio0_dma_area;
	}
	#endif
...
static int __devexit sunxi_daudio_pcm_remove(struct platform_device *pdev)
{
