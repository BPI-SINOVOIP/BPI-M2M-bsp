...
static int sunxi_i2s0_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	if (i2s0_select) {
		...
		switch (fmt & SND_SOC_DAIFMT_INV_MASK) {
		case SND_SOC_DAIFMT_NB_NF:     /* normal bit clock*/
			reg_val1 &= ~SUNXI_I2S0FAT0_BCP;
			break;
		case SND_SOC_DAIFMT_NB_IF:     /* normal bclk*/
			reg_val1 &= ~SUNXI_I2S0FAT0_BCP;
			break;
		case SND_SOC_DAIFMT_IB_NF:     /* invert bclk*/
			reg_val1 |= SUNXI_I2S0FAT0_BCP;
			break;
		case SND_SOC_DAIFMT_IB_IF:     /* invert bclk*/
			reg_val1 |= SUNXI_I2S0FAT0_BCP;
			break;
		}
	}
																...
																struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sample_resolution == 24) {
			...
			reg_val |= SUNXI_I2S0FCTL_RXOM_MOD1;
		}
		writel(reg_val, sunxi_i2s0.regs + SUNXI_I2S0FCTL);
	}
	/* play or record */
...
static int sunxi_i2s0_set_clkdiv(struct snd_soc_dai *cpu_dai, int div_id, int sample_rate)
{
	...
	if (i2s0_select) {
		...
		/*a=enable internal i2s global bit*/
		reg_val = readl(0xf1c22c00);
		reg_val |= 1<<0;
		writel(reg_val, 0xf1c22c00);

		/*enbale mclk && mclk = pll/2 = 12.228*/
...
static int __devinit sunxi_i2s0_dev_probe(struct platform_device *pdev)
{
	int ret = 0;
	int reg_val = 0;
	script_item_u val;
...
static int __devexit sunxi_i2s0_dev_remove(struct platform_device *pdev)
{
	if (i2s0_used) {
		i2s0_used = 0;
