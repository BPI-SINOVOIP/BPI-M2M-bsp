	...
	struct snd_soc_dai *dai)
{
	...
	reg_val = readl(SUNXI_I2S2_VBASE + SUNXI_I2S2CTL);
	reg_val |= SUNXI_I2S2CTL_GEN;
	writel(reg_val, SUNXI_I2S2_VBASE + SUNXI_I2S2CTL);
	msleep(10);
	/*flush TX FIFO*/
	reg_val = readl(SUNXI_I2S2_VBASE + SUNXI_I2S2FCTL);
    reg_val |= SUNXI_I2S2FCTL_FTX;
	writel(reg_val, SUNXI_I2S2_VBASE + SUNXI_I2S2FCTL);
#endif
#ifdef CONFIG_ARCH_SUN8IW7
	/*Global Enable Digital Audio Interface*/
