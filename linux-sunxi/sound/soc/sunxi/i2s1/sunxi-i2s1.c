																...
																struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if (sample_resolution == 24) {
			...
			reg_val |= SUNXI_I2S1FCTL_RXOM_MOD1;
		}
		writel(reg_val, sunxi_i2s1.regs + SUNXI_I2S1FCTL);
	}
	/* play or record */
...
static int __devinit sunxi_i2s1_dev_probe(struct platform_device *pdev)
{
	int ret = 0;
	int reg_val = 0;
	script_item_u val;
...
static int __devexit sunxi_i2s1_dev_remove(struct platform_device *pdev)
{
	if (i2s1_used) {
		i2s1_used = 0;
