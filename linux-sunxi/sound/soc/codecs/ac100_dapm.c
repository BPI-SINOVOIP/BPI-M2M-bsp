...
static void get_configuration(void)
{
	...
	type = script_get_item("ac100_audio0", "linein_to_spk_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
	...
	type = script_get_item("ac100_audio0", "linein_to_hp_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
		pr_err("[audiocodec] linein_to_hp_used type err!\n");
	else
		linein_to_hp_used = val.val;

	type = script_get_item("ac100_audio0", "linein_to_aif2_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
	...
	if (linein_to_spk_used || linein_to_aif2_used || linein_to_hp_used) {
		type = script_get_item("ac100_audio0",\
					"audio_lowlevel_detect", &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
		...
		*/
		...
		if (!ret) {
			if (linein_plugin == false) {
				...
				* linein_to_aif2 is use for
				* i2s+codec linein function detect.
				* linein_to_aif2 don't use snd_jack_report here,
				* it should be realize at codec driver.
				*/
			...
			if (linein_plugin == true) {
				...
				* it should be realize at codec driver.
				*/
				if (linein_to_spk_used || linein_to_hp_used) {
					snd_jack_report(jack.jack,\
		...
		while (irq_flag == 0) {
			...
			if(ac100->check_count_sum <= HEADSET_CHECKCOUNT_SUM){
				...
				if (temp_value[ac100->check_count -2] >= 0xb) {
					...
					ac100->check_count_sum = 0;
					reset_flag = 0;
					input_report_key(ac100->key, KEY_HP, 1);
					input_sync(ac100->key);
					break;
					...
					ac100->check_count_sum = 0;
					reset_flag = 0;
					input_report_key(ac100->key, KEY_HP, 0);
					input_sync(ac100->key);
					break;
				}
			}
...
static int ac100_codec_probe(struct snd_soc_codec *codec)
{
	...
	set_bit(KEY_HP, ac100->key->keybit);
