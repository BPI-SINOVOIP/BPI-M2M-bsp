...
static void ac108_hw_init(struct i2c_client *i2c)
{
	...
	ac108_update_bits(I2S_LRCK_CTRL1, 0x3 << LRCK_PERIODH,
			((ac108_pub_cfg.slot_width - 1) >> 8) << LRCK_PERIODH, i2c);
	ac108_write(I2S_LRCK_CTRL2, (u8) (ac108_pub_cfg.slot_width - 1), i2c);
	...
	ac108_update_bits(I2S_FMT_CTRL2, 0x7 << SLOT_WIDTH_SEL,\
			(ac108_pub_cfg.slot_width / 4 - 1) << SLOT_WIDTH_SEL, i2c);
...
static int ac108_set_clkdiv(struct snd_soc_dai *dai, int div_id, int div)
{
	...
	bclk_div = div / (ac108_pub_cfg.slot_width * (AC108_ENCODING_EN ?\
...
static int ac108_probe(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_codec_set_cache_io(codec, ac108_regmap_config.reg_bits,\
			ac108_regmap_config.val_bits, CONFIG_REGMAP_I2C);
	/* //8,8 */
#else
	codec->control_data =
		...
		const struct i2c_device_id *i2c_id)
{
	...
	type = script_get_item("ac108", "ac108_power", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_STR != type) {
		pr_err("[I2S0] ac108_power type err!\n");
	} else {
		if (!regulator_en) {
			ac108_pub_cfg.power_name = val.str;
			type = script_get_item("ac108", "ac108_power_vol", &val);
			if (SCIRPT_ITEM_VALUE_TYPE_INT == type) {
				ac108_pub_cfg.power_vol = val.val;
				...
				regulator_set_voltage(ac108->vol_supply.vcc3v3, ac108_pub_cfg.power_vol,
		...
		struct i2c_board_info *info)
{
	...
	if (adapter->nr == ac108_pub_cfg.i2c_bus_num) {
		ac108_read(CHIP_AUDIO_RST, &reg, client);
		if (reg == 0x4a) {
			if (client->addr == 0x3b) {
...
static int __init ac108_init(void)
{
	...
	type = script_get_item("ac108", "pga_gain", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] get pga_gain err!\n");
	}
	ac108_pub_cfg.pga_gain = val.val;

	type = script_get_item("ac108", "ref_pga_gain", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] get ref_pga_gain err!\n");
	}
	ac108_pub_cfg.ref_pga_gain = val.val;
	type = script_get_item("ac108", "slot_width", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] get slot_width err!\n");
	}
	ac108_pub_cfg.slot_width = val.val;

	type = script_get_item("ac108", "ref_chip_addr", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] get ref_chip_addr err!\n");
	}
	ac108_pub_cfg.ref_chip_addr = val.val;

	type = script_get_item("ac108", "ref_channel_num", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] get ref_channel_num err!\n");
	}
	ac108_pub_cfg.ref_channel_num = val.val;

	type = script_get_item("ac108", "pa_double_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108]get pa_double_used err!\n");
	}
	ac108_pub_cfg.pa_double_used = val.val;

	type = script_get_item("ac108", "codec_mic_used", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108]get codec_mic_used err!\n");
	}
	ac108_pub_cfg.codec_mic_used = val.val;

	type = script_get_item("ac108", "i2c_bus_num", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		pr_err("[ac108] i2c_bus_num err!\n");
	}
	ac108_pub_cfg.i2c_bus_num = val.val;
