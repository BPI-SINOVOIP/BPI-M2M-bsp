...
static unsigned int nau8540_read(struct snd_soc_codec *codec, unsigned int reg)
{
	struct nau8540 *nau8540 = snd_soc_codec_get_drvdata(codec);
	unsigned int ret, val;

	ret = regmap_read(nau8540->regmap, reg, &val);
	if (ret < 0) {
		pr_err("[nau8540] Failed to read value from reg %x\n", reg);
		return ret;
	}
...
static int nau8540_write(struct snd_soc_codec *codec,
		unsigned int reg, unsigned int val)
{
	struct nau8540 *nau8540 = snd_soc_codec_get_drvdata(codec);
	unsigned int ret;

	ret = regmap_write(nau8540->regmap, reg, val);
	if (ret < 0)
		pr_err("[nau8540] Failed to write value %x to reg %x\n",
				val, reg);
...
static struct i2c_board_info nau8540_i2c_board_info[] = {
...
static int __init nau8540_init(void)
{
	...
	type = script_get_item(NAU8540_NAME, "i2c_bus_number", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type)
		pr_err("[%s] i2c_bus_number type err!\n");
	i2c_bus_number = val.val;

	adapter = i2c_get_adapter(i2c_bus_number);
	if (!adapter) {
		pr_err("[nau8540] i2c_get_adapter for I2C channel %d failed!\n",
				i2c_bus_number);
		return -ENODEV;
	}
	for (i = 0; i < sizeof(nau8540_i2c_board_info)/\
			sizeof(nau8540_i2c_board_info[0]); ++i) {
		client = NULL;
		client = i2c_new_device(adapter, &nau8540_i2c_board_info[i]);
		if (!client) {
			pr_err("[nau8540] i2c_new_device failed! (type: %s, addr: %d)\n",
					nau8540_i2c_board_info[i].type,
					nau8540_i2c_board_info[i].addr);
			return -ENODEV;
		}
	}
	i2c_put_adapter(adapter);

	return i2c_add_driver(&nau8540_i2c_driver);
}
module_init(nau8540_init);

static void __exit nau8540_exit(void)
{
	i2c_del_driver(&nau8540_i2c_driver);
}
module_exit(nau8540_exit);
