	...
	*drv_priv = priv;
	...
	if (atomic_read(&hw_priv->num_vifs) < XRWL_MAX_VIFS) {
		...
		if(priv->mode == NL80211_IFTYPE_MONITOR){
			spin_unlock(&hw_priv->vif_list_lock);
			mutex_unlock(&hw_priv->conf_mutex);
			sta_printk(XRADIO_DBG_WARN, "%s: NL80211_IFTYPE_MONITOR unsupport!\n",
			           __func__);
			return -EOPNOTSUPP;
		}
#endif
...
int xradio_config(struct ieee80211_hw *dev, u32 changed)
{
	...
	if(hw_priv->monitor_if_id != -1)
		priv = __xrwl_hwpriv_to_vifpriv(hw_priv, hw_priv->monitor_if_id);
	else
#endif
	...
	if ((changed & IEEE80211_CONF_CHANGE_CHANNEL) &&
		...
		int prev_ch = hw_priv->channel ? hw_priv->channel->hw_value : -1;
		sta_printk(XRADIO_DBG_WARN, "Freq %d (wsm ch: %d) prev: %d.\n",
		           ch->center_freq, ch->hw_value, prev_ch);
		/* Earlier there was a call to __xradio_flush().
		   Removed as deemed unnecessary */
			hw_priv->channel = ch;
			hw_priv->channel_changed = 1;
#ifdef MONITOR_MODE
		if(priv->mode == NL80211_IFTYPE_MONITOR && priv->join_status != XRADIO_JOIN_STATUS_PASSIVE)
...
void xradio_flush(struct ieee80211_hw *hw, struct ieee80211_vif *vif, bool drop)
{
	struct xradio_common *hw_priv = hw->priv;
	struct xradio_vif *priv = xrwl_get_vif_from_ieee80211(vif);
	sta_printk(XRADIO_DBG_TRC,"%s\n", __func__);
...
#ifdef MONITOR_MODE
int xradio_enable_monitoring(struct xradio_vif *priv, struct ieee80211_channel *chan){
...
#else
		ret = xradio_queue_remove(hw_priv, queue, hw_priv->pending_frame_id);
#endif
		if (ret)
...
#ifdef CONFIG_XRADIO_TESTMODE
		xradio_queue_requeue(queue,
			hw_priv->pending_frame_id, false);
#else
		xradio_queue_requeue(hw_priv, queue, hw_priv->pending_frame_id, false);
#endif
...
#else
			xradio_queue_requeue(hw_priv, queue, hw_priv->pending_frame_id,
						true);
#endif
			priv->join_status = XRADIO_JOIN_STATUS_STA;
