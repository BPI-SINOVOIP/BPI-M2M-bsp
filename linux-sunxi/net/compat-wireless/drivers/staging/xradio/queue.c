				     ...
				     struct xradio_queue_item *item)
{
	...
	gc_item = kmalloc(sizeof(struct xradio_queue_item),
			GFP_ATOMIC);
	SYS_BUG(!gc_item);
...
#else
int xradio_queue_requeue(struct xradio_common *hw_priv, struct xradio_queue *queue, u32 packetID, bool check)
#endif
{
	int ret = 0;
...
#else
int xradio_queue_remove(struct xradio_common *hw_priv, struct xradio_queue *queue, u32 packetID)
#endif /*CONFIG_XRADIO_TESTMODE*/
{
	int ret = 0;
	u8 queue_generation, queue_id, item_generation, item_id, if_id, link_id;
