...
#ifdef XR819_HT_ADD_Q
	{
		u8* llc_data  = skb->data+ieee80211_hdrlen(frame->frame_control);
		u8* ip_hdr    = llc_data+LLC_LEN;
		u8* ipaddr    = ip_hdr+IP_D_ADD_OFF;
...
#ifdef XR819_HT_ADD_Q
	if (set_tx_event) {
		if (atomic_add_return(1, &hw_priv->bh_tx) >= 10)
...
drop:
	txrx_printk(XRADIO_DBG_ALWY, "drop=%d, fctl=0x%04x.\n", ret, frame->frame_control);
