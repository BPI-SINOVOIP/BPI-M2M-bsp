...
static struct sk_buff *xradio_get_skb(struct xradio_common *hw_priv, size_t len)
{
	...
	if (len > SDIO_BLOCK_SIZE || !hw_priv->skb_cache) {
		...
		if (skb) {
			...
			bh_printk(XRADIO_DBG_ERROR,"%s dev_alloc_skb failed!\n", __FUNCTION__);
		}
	} else {
	...
	for (;;) {
		...
		if (!status) {
			...
			if (hw_priv->hw_bufs_used) {
				...
				if (!hw_priv->device_can_sleep && !atomic_read(&hw_priv->recent_scan)) {
		...
		/* 3--Host suspend request. */
			...
			if (hw_priv->powersave_enabled) {
				bh_printk(XRADIO_DBG_MSG, "Device idle(host suspend), can sleep.\n");
				SYS_WARN(xradio_reg_write_16(hw_priv, HIF_CONTROL_REG_ID, 0));
				hw_priv->device_can_sleep = true;
			}
...
#endif
			if (status < 0) {
				bh_printk(XRADIO_DBG_ERROR,"Failed to wait for resume: %ld.\n", status);
				hw_priv->bh_error = __LINE__;
				break;
			}
...
rx:
			...
			if (WARN_ON_ONCE(alloc_len > EFFECTIVE_BUF_SIZE)) {
				bh_printk(XRADIO_DBG_ERROR, "Read aligned len: %d\n", alloc_len);
			}
