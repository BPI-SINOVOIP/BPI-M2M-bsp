...
/* ******************************************************************** */
//add by yangfh, forcing upper layer to restart wifi.
			    ...
			    struct wsm_buf *buf)
{
	...
	if (WSM_GET32(buf) != WSM_STATUS_SUCCESS) //if (SYS_WARN(WSM_GET32(buf) != WSM_STATUS_SUCCESS))
		return -EINVAL;
	...
	if (priv->join_status == XRADIO_JOIN_STATUS_AP) {
		...
		for (i = 0; i<MAX_STA_IN_AP_MODE; i++) {
			if (priv->link_id_db[i].status == XRADIO_LINK_HARD) {
				...
				deauth->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_DEAUTH |
				                                    IEEE80211_FCTL_TODS);
				deauth->duration = 0;
	...
	if (priv->join_status == XRADIO_JOIN_STATUS_AP) {
		...
		for (i = 0; i<MAX_STA_IN_AP_MODE; i++) {
			if (priv->link_id_db[i].status == XRADIO_LINK_HARD) {
				...
				disassoc->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_DISASSOC |
				                                    IEEE80211_FCTL_TODS);
				disassoc->duration = 0;
				...
				int interface_link_id)
{
	...
	if (event == NULL) {
		wsm_printk(XRADIO_DBG_ERROR, "%s:kzalloc failed!", __func__);
		return -EINVAL;
	}
		 ...
		 void *arg, u16 cmd, long tmo, int if_id)
{
	...
	if (unlikely(ret == 0)) {
		...
		wsm_printk(XRADIO_DBG_ERROR, "***CMD timeout!>>> 0x%.4X (%d), buf_use=%d\n",
...
void wsm_buf_init(struct wsm_buf *buf)
{
	...
	SYS_BUG(buf->begin);
	buf->begin = kmalloc(size, GFP_KERNEL | GFP_DMA);
...
static struct xradio_vif * wsm_get_interface_for_tx(struct xradio_common *hw_priv)
{
