...
static bool debug_enable = 0;
static bool dump_freq    = 0;
extern int mali_freq;
...
int mali_sysfs_register(const char *mali_dev_name)
{
	...
	if(ERR_PTR(-ENODEV) == mali_debugfs_dir) {
		...
		if(NULL != mali_debugfs_dir) {
			...
			debugfs_create_file("dump", 0400, mali_debugfs_dir, NULL, &dump_fops);
			debugfs_create_file("write", 0200, mali_debugfs_dir, NULL, &write_fops);
		}
	}
