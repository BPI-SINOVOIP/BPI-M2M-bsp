...
__s32	Uboot_Start_Block;
__s32	Uboot_Next_Block;
...
int NAND_BurnBoot1(unsigned int length, void *buf)
{
	...
	buffer = (void *)kmalloc(length+32*1024, GFP_KERNEL);
	if (buffer == NULL)
	{
		debug("no memory!\n");
		NAND_PhysicUnLock();
	...
	package_size = Physic_Info_Add_To_Uboot_Tail(buffer, length);
	...
	if(package_size % page_size)
		package_size = (package_size%page_size+1)*page_size;
	...
	{
		ret = burn_boot1_in_one_blk((__u32)buffer, package_size);
		debug("NAND_BurnBoot1, in one blk, %d\n", ret);
	}
	else
	{
		ret = burn_boot1_in_many_blks((__u32)buffer, package_size);
