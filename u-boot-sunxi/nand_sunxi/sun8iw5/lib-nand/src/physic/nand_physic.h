...
//define if lsb type of hynix nand
#define HYINX_NAND_LSB_PAGE_TYPE			((NAND_LSB_PAGE_TYPE & NandStorageInfo.OperationOpt)>>16)

//define if the nand flash requre to user max block erase cnt
#define SUPPORT_USE_MAX_BLK_ERASE_CNT		(NAND_MAX_BLK_ERASE_CNT & NandStorageInfo.OperationOpt)

//define if the nand flash requre to do read reclaim operation
#define SUPPORT_READ_RECLAIM				(NAND_READ_RECLAIM & NandStorageInfo.OperationOpt)



//define if the nand flash require to change timing mode according to ONFI 2.x/3.0
#define SUPPORT_CHANGE_ONFI_TIMING_MODE		(NAND_TIMING_MODE & NandStorageInfo.OperationOpt)

//define if the nand flash require to set ddr2 specific feature according to ONFI 3.0 or Toggle DDR2
#define SUPPORT_DDR2_SPECIFIC_CFG			(NAND_DDR2_CFG & NandStorageInfo.OperationOpt)

//define if the nand flash require to set io driver strength according to ONFI 2.x/3.0 or Toggle DDR1/DDR2
#define SUPPORT_IO_DRIVER_STRENGTH			(NAND_IO_DRIVER_STRENGTH & NandStorageInfo.OperationOpt)

//define if the nand flash require to set vendor's specific configuration
#define SUPPORT_VENDOR_SPECIFIC_CFG			(NAND_VENDOR_SPECIFIC_CFG & NandStorageInfo.OperationOpt)

//define if the nand flash require to onfi sync reset
#define SUPPORT_SYNC_RESET_OP				(NAND_ONFI_SYNC_RESET_OP & NandStorageInfo.OperationOpt)

//define if the nand flash DO NOT support to switch interface between legacy and toggle
#define SUPPORT_TOGGLE_ONLY                 (NAND_TOGGLE_ONLY & NandStorageInfo.OperationOpt)
...
*/
__s32 PHY_Init(void);
__s32 PHY_ChangeMode(__u32 ddr_type, void *ddr_info, __u32 sclk0, __u32 sclk1);
