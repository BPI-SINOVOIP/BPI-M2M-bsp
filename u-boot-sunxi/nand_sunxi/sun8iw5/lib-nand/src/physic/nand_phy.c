...
__u8* Nand_Sorting_Badblk_Info[MAX_CHIP_SELECT_CNT]={NULL};


//init for nand sorting
__u32 Support_nand_sorting = 0;
...
__s32 _read_single_page_spare(struct boot_physical_param *readop,__u8 dma_wait_mode)
{
    ...
    {
        if((READ_RETRY_MODE>=0x00)&&(READ_RETRY_MODE<0x60))  //toshiba & Samsung mode & Sandisk mode & micron mode & intel mode
		{
	...
	for(NandIndex = 0; NandIndex<CHANNEL_CNT;NandIndex++)
	{
		...
		else if (SECTOR_CNT_OF_SINGLE_PAGE == 4)
		{
			if(Two_Row_Addr_Flag)
			{
				for (i = 0; i < plane_cnt; i++){
					_cal_addr_in_chip(block_in_chip+ i*MULTI_PLANE_BLOCK_OFFSET,0, 0,addr[i], 3);
					_add_cmd_list(cmd_list+2*i,0x60,2,addr[i],NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE);
					_add_cmd_list(cmd_list + 2*i + 1,0xd0,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_WAIT_RB);
				}
			}
			else
			{
				for (i = 0; i < plane_cnt; i++){
					_cal_addr_in_chip(block_in_chip+ i*MULTI_PLANE_BLOCK_OFFSET,0, 0,addr[i], 3);
					_add_cmd_list(cmd_list+2*i,0x60,3,addr[i],NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE);
					_add_cmd_list(cmd_list + 2*i + 1,0xd0,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_WAIT_RB);
				}
			}
			list_len = 2 * plane_cnt;
		}
...
__s32 _read_sectors_first_new(struct boot_physical_param *readop,__u8 dma_wait_mode)
{
	...
	if(Two_Row_Addr_Flag)
	{
		_cal_addr_in_chip(readop->block,readop->page,0,pre_addr,4);
		_add_cmd_list(&pre_cmd_list[0],0x00,4,pre_addr,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE,NDFC_IGNORE);
	}
	else
	{
		//cmd prepair
...
__s32 _read_sectors_v2_first(struct boot_physical_param *readop, __u8 dma_wait_mode)
{
    ...
    if(SUPPORT_READ_RETRY)
    {
        if((READ_RETRY_MODE>=0x0)&&(READ_RETRY_MODE<0x60))  //toshiba & Samsung mode & Sandisk mode & micron mode & intel mode
        {
			RetryCount[NandIndex][readop->chip] = 0;
			if((READ_RETRY_MODE>=0x30)&&(READ_RETRY_MODE<0x40))  //Sandisk mode
			{
...
__s32  PHY_PageRead(struct __PhysicOpPara_t *pPageAdr)
{
	...
	__u32 bad_flag =0;
	__u32 validbit = 0, continuous_flag = 0; //firstbit = 0, 
	__u32 dbg = 1;
	__u32 data_size_first, data_size_wait;
	//__u32 err, cnt;
	...
	for (i=0; i<plane_cnt; i++)
	{
		...
		for (NandIndex=0; NandIndex<CHANNEL_CNT; NandIndex++)
		{
			...
			{
				//firstbit = _cal_first_valid_bit(bitmap_in_single_page);
				validbit = _cal_valid_bits(bitmap_in_single_page);
				continuous_flag = _check_continuous_bits(bitmap_in_single_page);
				if (continuous_flag == 0) {
					PHY_ERR("PHY_PageRead: error bitmap(0x%x) in physic page: \n", bitmap_in_single_page);
					PHY_ERR("	plane:%d, ch:%d, chip:%x, blk:0x%x, pg:0x%x\n",
							i, j, readop.chip, readop.block, readop.page);

					ret[NandIndex][i] |= -ERR_TIMEOUT;
					continue;
				}

				readop.mainbuf = (__u8 *)(pPageAdr->MDataPtr) + data_size_first;
		...
		for(NandIndex=0; NandIndex<CHANNEL_CNT; NandIndex++)
		{
			...
			if (bitmap_in_single_page)
			{
				...
				readop.mainbuf = (__u8 *)(pPageAdr->MDataPtr) + data_size_wait;
...
//		}
		...
		{
			MEMCPY((__u8 *)pPageAdr->SDataPtr, &tmp_oob[0][0][0], 8);
			MEMCPY(((__u8 *)pPageAdr->SDataPtr + 8), &tmp_oob[1][0][1], 7);
		}
#endif
...
__s32 _read_sectors_for_spare(struct boot_physical_param *readop,__u8 dma_wait_mode)
{
	...
	__u32 column;//,retry_judge_limit;
	...
	{
		_cal_addr_in_chip(readop->block,readop->page, 0, addr, 4);
		_add_cmd_list(cmd_list,0x00,4,addr, NDFC_NO_DATA_FETCH,NDFC_IGNORE,NDFC_IGNORE, NDFC_IGNORE);
	}
	else
	{
		_cal_addr_in_chip(readop->block,readop->page, 0, addr, 5);
...
__s32 PHY_Nand_Sorting_scan_badblock_info(__u32* address)
{
	...
	start_block = 30;
	block_num = 10;
	/*ecc_mode = NandStorageInfo.EccMode;*/
	/*lsb_page_type = 0xff;*/

	for(chip_cnt=0;chip_cnt<NandStorageInfo.ChipCnt;chip_cnt++)
	{
		chip = _cal_real_chip(chip_cnt);
		readop.chip = chip;
		readop.block = 12;
		readop.page = 0;
		readop.mainbuf = (void *)PageCachePool.TmpPageCache;
		readop.oobbuf = (void *)oob_buf;
		for(blk=start_block;blk<(start_block + block_num);blk++)
		{
			readop.block = blk;
			...
			if((oob_buf[0] != 0x25522500)&&(oob_buf[1] != 0x25522552))
			{
				ret = 1;
				continue;
			}
			...
			if(ret==0)
			{
				PHY_DBG("find good bad block table,chip %d ,block %d \n",chip,blk);
				
				chip_addr[chip_cnt] = (__u32)MALLOC(NandStorageInfo.BlkCntPerDie*NandStorageInfo.DieCntPerChip); 
				MEMCPY((void *)(chip_addr[chip_cnt]),(void *)readop.mainbuf,NandStorageInfo.BlkCntPerDie*NandStorageInfo.DieCntPerChip);

				//set the table block as bad bad block
				((__u8 *)(chip_addr[chip_cnt]))[blk]=0x1;
				
				address[chip_cnt] = chip_addr[chip_cnt];
				cnt++;
				break;
			}
		}
		if(ret!=0)
		{
			PRINT("can't find good bad block table on chip %d \n",chip);
		}	
	}
	return cnt;
...
__s32 PHY_NAND_Sorting_Init(void)
{
	__u32 badblk_info_address[MAX_CHIP_SELECT_CNT]={0};
	__s32 cnt,i;

	for(i=0;i<MAX_CHIP_SELECT_CNT;i++)
		badblk_info_address[i]=0;

	cnt = PHY_Nand_Sorting_scan_badblock_info(&badblk_info_address[0]);
	if(cnt==0)
	{
		return -1;
	}
	for(i=0;i<MAX_CHIP_SELECT_CNT;i++)
	{
		Nand_Sorting_Badblk_Info[i] = NULL;
		if(badblk_info_address[i]!=0)
		{
			Nand_Sorting_Badblk_Info[i] = (__u8 *)(badblk_info_address[i]);
		}
	}
	Support_nand_sorting = 1;
...
__s32 PHY_Nand_Sorting_IsBadblock(__u32 chip_num,__u32 blk_num)
{
	...
	{
		page_size = NandStorageInfo.BlkCntPerDie*NandStorageInfo.DieCntPerChip;
			
		if(blk_num>=page_size)
		{
			PRINT("[NAND SORTING]invalid blk num\n");
			return -1;
		}
		if(Nand_Sorting_Badblk_Info[chip_num]!=NULL)
		{
			if((Nand_Sorting_Badblk_Info[chip_num])[blk_num]==0x1)
			{
				PRINT("nand sorting badblk info %x, blk num %x  \n",(Nand_Sorting_Badblk_Info[chip_num])[blk_num],blk_num);
				bad_flag = 1;
			}
		}
	}
...
__s32  PHY_Nand_Sorting_IsBadblock_superblk(struct __PhysicOpPara_t *pPageAdr)
{
	__s32 ret;
	__u32 block_in_chip;
	__u32 plane_cnt,i;
	...
	if(Support_nand_sorting==1)
	{
		...
		block_in_chip = _cal_block_in_chip(pPageAdr->BankNum,pPageAdr->BlkNum);
		if (0xffffffff == block_in_chip){
			PHY_ERR("PHY_Nand_Sorting : beyond block of per chip  count\n");
			return -ERR_INVALIDPHYADDR;
		}

		for (i = 0; i < plane_cnt; i++){
			
			block = block_in_chip  + i*MULTI_PLANE_BLOCK_OFFSET;

			ret = PHY_Nand_Sorting_IsBadblock(pPageAdr->BankNum,block);
			if(ret == -1)
			{
				PRINT("PHY_Nand_Sorting_IsBadblock operation error\n");
				return -1;
			}
			bad_flag |= ret;
		}
