...
{
	u8    pmu_type;
	axp_i2c_config(SUNXI_AXP_259, AXP259_ADDR);
...
int axp259_probe_battery_vol(void)
{
    ...
    int bat_vol, temp_value;
    if(axp_i2c_read(AXP259_ADDR, BOOT_POWER259_BAT_VOL_H8, &reg_value_h))
    {
        return -1;
    }
    if(axp_i2c_read(AXP259_ADDR, BOOT_POWER259_BAT_VOL_L4, &reg_value_l))
    {
        return -1;
    }
    //check spec: 6.7 ADC
    temp_value = (reg_value_h << 4) | reg_value_l;
    bat_vol = temp_value * 12;
    bat_vol /= 10;
...
{
	u8  reg_value;
	if(axp_i2c_read(AXP259_ADDR, BOOT_POWER259_DATA_BUFFER10, &reg_value))
	{
		return -1;
	}
...
{
	if(axp_i2c_write(AXP259_ADDR, BOOT_POWER259_DATA_BUFFER10, (u8)data))
	{
		return -1;
	}
	return 0;
