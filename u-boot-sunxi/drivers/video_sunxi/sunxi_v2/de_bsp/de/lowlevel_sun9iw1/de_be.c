...
u32 image_reg_base[3] = {0,0,0};//DISE_REGS_BASE;
...
s32 DE_BE_Set_SystemPalette(u32 sel, u32 * pbuffer, u32 offset,u32 size)
{
	...
	psrc_cur = pbuffer;
	pdest_cur = (u32*)(DE_Get_Reg_Base(sel)+DE_BE_PALETTE_TABLE_ADDR_OFF  + offset);
	pdest_end = pdest_cur + (size>>2);

    while(pdest_cur < pdest_end)
    {
    	*(volatile u32 *)pdest_cur++ = *psrc_cur++;
    }
...
s32 DE_BE_Get_SystemPalette(u32 sel, u32 *pbuffer, u32 offset,u32 size)
{
	...
	psrc_cur = (u32*)(DE_Get_Reg_Base(sel)+DE_BE_PALETTE_TABLE_ADDR_OFF + offset);
	pdest_cur = pbuffer;
	pdest_end = pdest_cur + (size>>2);
...
s32 DE_BE_Sprite_Block_Set_Size(u32 sel, u8 blk_idx,u32 xsize,u32 ysize)//todo
{
	u32 tmp = 0;
...
s32 DE_BE_Sprite_Set_Palette_Table(u32 sel, u32 address, u32 offset, u32 size)
{
	...
	psrc_cur = (u32*)address;
	pdest_cur = (u32*)(DE_Get_Reg_Base(sel) + DE_BE_SPRITE_PALETTE_TABLE_ADDR_OFF + offset);
	pdest_end = pdest_cur + (size>>2);
...
s32 DE_BE_Set_Enhance_ex(u8 sel, u32 out_csc, u32 out_color_range, u32 enhance_en, u32 brightness, u32 contrast, u32 saturaion, u32 hue)
{
	...
	i_bright = (s32)(brightness*64/100);
	i_saturaion = (s32)(saturaion*64/100);
	i_contrast = (s32)(contrast*64/100);
	i_hue = (s32)(hue*64/100);
	...
	for(i=0;i<4;i++)
	{
		DE_BE_WUINT32(sel, DE_BE_OUT_COLOR_R_COEFF_OFF+ 4*i, (u32 )(*(pt + i)));
	    DE_BE_WUINT32(sel, DE_BE_OUT_COLOR_G_COEFF_OFF+ 4*i, (u32 )(*(pt + 4 + i)));
	    DE_BE_WUINT32(sel, DE_BE_OUT_COLOR_B_COEFF_OFF+ 4*i, (u32 )(*(pt + 8 + i)));
	}
...
s32 DE_BE_set_display_size(u32 sel, u32 width, u32 height)
{
    ...
    DE_BE_WUINT32(sel, DE_BE_DISP_SIZE_OFF, ((height)<<16) | (width));
    return 0;
}
