...
/*
s32 tcon_set_start_delay(u32 sel, u32 tcon_index, u8 delay)
{
    if(tcon_index == 0)
    {
...
s32 tcon1_cfg(u32 sel,disp_video_timing* timing)
{
	...
	lcd_dev[sel]->tcon1_basic0.bits.y = timing->y_res / (timing->interlace+1) - 1;
	lcd_dev[sel]->tcon1_basic1.bits.ls_xo = timing->x_res - 1;
	lcd_dev[sel]->tcon1_basic1.bits.ls_yo = timing->y_res / (timing->interlace+1) + (timing->trd_mode? timing->vactive_space:0)-1;
	lcd_dev[sel]->tcon1_basic2.bits.xo = timing->x_res - 1;
	lcd_dev[sel]->tcon1_basic2.bits.yo = timing->y_res / (timing->interlace+1) + (timing->trd_mode? timing->vactive_space:0)-1;
	...
	start_delay = (timing->ver_total_time - timing->y_res) / (timing->interlace+1) - 5;
...
u32 tcon_ceu_range_cut(s32 *x_value,s32 x_min,s32 x_max)
{
	if(*x_value>x_max)
	{
		*x_value = x_max;
...
s32 tcon_ceu_reg_corr(s32 val,u32 bit)
{
	if(val>=0)
		return val;
	else
...
s32 tcon_ceu_rect_multi(s32 *dest, s32 *src1, s32 *src2)
{
	u32 x,y,z;
	__s64 val_int64;
...
s32 tcon_ceu_rect_calc(s32 *p_rect,s32 b,s32 c,s32 s,s32 h)
{
...
s32 tcon_ceu_calc(u32 r2y_type,u32 cen_type,u32 y2r_type, s32 b,s32 c,s32 s,s32 h,s32* p_coff)
{
	const s32 rect_1[16]=
	{
		1024,	   0,	   0,	   0,
...
s32 tcon_cmap(u32 sel, u32 mode,disp_lcd_cmap_color lcd_cmap_tbl[2][3][4])
{
	if(!(mode==1))
	{
