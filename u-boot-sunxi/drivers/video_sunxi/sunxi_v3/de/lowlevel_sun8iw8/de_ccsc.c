...
static int de_ccsc_set_reg_base(unsigned int sel, unsigned int chno, unsigned int base)
{
...
static int de_icsc_set_reg_base(unsigned int sel, unsigned int chno, unsigned int base)
{
	__inf("sel=%d, chno=%d, base=0x%x\n", sel, chno, base);
...
int de_ccsc_init(disp_bsp_init_para *para)
{
	...
	for(screen_id=0; screen_id<device_num; screen_id++)
		for(ch_id=0; ch_id<vi_num[screen_id]; ch_id++)
		{
			...
			csc_block[screen_id][ch_id].dirty 		= 1;//Sram to Register should init dirty flag to zero

			de_ccsc_set_reg_base(screen_id, ch_id, (unsigned int)memory);
			...
			if(vep_support[screen_id][ch_id])
			{
				...
				de_icsc_set_reg_base(screen_id, ch_id, (unsigned int)memory);
			}
