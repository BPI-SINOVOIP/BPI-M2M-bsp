					...
					unsigned int *slope0, unsigned int *slope1, unsigned int *slope2, unsigned int *slope3)
{
	...
	if(frame_bld_en)
	{
		...
		weight = (coeff >= bld_high_thr)? 256 :
			...
			((256 - bld_low_thr)/(bld_high_thr - bld_low_thr) * (coeff - bld_low_thr) + bld_weight_lmt);
	...
	if(validcnt != 0)
	{
		...
		if(		((tmp = (	pd_black + ((pd_s1 * (pd_ymin - pd_black))	>>8	))) >=0)	&& (pd_ymin <= pd_black) && (pd_ymin > 0))
		{
		...
		if(		((tmp = ( pd_white + ((pd_s2 * (pd_ymax - pd_white)		)>>8)) ) <= 255)	&& (pd_ymax >= pd_white)  && (pd_ymax < 255))
		{
