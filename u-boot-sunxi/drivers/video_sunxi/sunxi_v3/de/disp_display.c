...
s32 disp_init_connections(void)
{
	...
	for (disp = 0; disp < num_screens; disp++) {
		...
		if (bsp_disp_feat_is_supported_output_types(disp,
...
void sync_event_proc(u32 disp)
{
	...
	*/
	...
	spin_unlock_irqrestore(&gdisp.screen[disp].flag_lock, flags);
		...
		if (gdisp.screen[disp].vsync_event_en &&
...
s32 bsp_disp_get_screen_width_from_output_type(u32 disp,
