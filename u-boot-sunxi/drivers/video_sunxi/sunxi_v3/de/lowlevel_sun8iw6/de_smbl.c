...
static unsigned int PWRSAVE_CORE(u32 sel)
{
	...
	u32 min_adj_index;
	u32 lgcaddr;
	u32 drc_filter_total=0, drc_filter_tmp=0;
...
int de_smbl_apply(unsigned int sel, struct disp_smbl_info *info)
{
	...
	if(info->flags & SMBL_DIRTY_ENABLE)
	{
		...
		if(g_smbl_status[sel]->IsEnable)
		{
			...
			memset(g_smbl_status[sel]->min_adj_index_hist, 255, sizeof(__u8)*IEP_LH_PWRSV_NUM);
...
int de_smbl_update_regs(unsigned int sel)
{
	...
	if(smbl_csc_block[sel].dirty == 0x1){
		memcpy((void *)smbl_csc_block[sel].off,smbl_csc_block[sel].val,smbl_csc_block[sel].size);
		smbl_csc_block[sel].dirty = 0x0;}
	if(smbl_filter_block[sel].dirty == 0x1){
		memcpy((void *)smbl_filter_block[sel].off,smbl_filter_block[sel].val,smbl_filter_block[sel].size);
		smbl_filter_block[sel].dirty = 0x0;}
...
int de_smbl_init(unsigned int sel, unsigned int reg_base)
{
	...
	smbl_ctrl_block[sel].off			= base;
	smbl_ctrl_block[sel].val			= memory;
	smbl_ctrl_block[sel].size			= 0x3c;
...
int de_smbl_set_para(unsigned int sel, unsigned int width, unsigned int height)
{
	...
	//out_csc coeff
	memcpy((void*)smbl_csc_block[sel].val, (unsigned char *)csc_bypass_coeff, sizeof(unsigned int)*12);

	//filter coeff
	memcpy((void*)smbl_filter_block[sel].val, (unsigned char *)smbl_filter_coeff, sizeof(unsigned char)*272);

	smbl_ctrl_block[sel].dirty = 1;
	smbl_csc_block[sel].dirty = 1;
	smbl_filter_block[sel].dirty = 1;
