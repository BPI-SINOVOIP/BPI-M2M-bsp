/*
 * Allwinner SoCs display driver.
 *
 * Copyright (C) 2016 Allwinner.
 *
 * This file is licensed under the terms of the GNU General Public
 * License version 2.  This program is licensed "as is" without any
 * warranty of any kind, whether express or implied.
 */

#include "de_smbl.h"

#if defined(__LINUX_PLAT__)
__u16 pwrsv_lgc_tab[1408][256] =
{
	{0}
};
#else
__u16 pwrsv_lgc_tab[1][256] =
{
	//gamma = 1.0
	//tab index : 128
	{0}
};
#endif


//__u8 spatial_coeff[9]={228,241,228,241,255,241,228,241,228};

__u8 smbl_filter_coeff[272]={
228, 241, 228, 0,   241, 255, 241, 0,   228, 241, 228, 0,   0,   0,   0,   0,
};

__u8 hist_thres_drc[8]={25,30,50,70,110,180,200,255};
__u8 hist_thres_pwrsv[8] = {128,147,175,199,219,235,247,255};
__u8 drc_filter[IEP_LH_PWRSV_NUM] = {9,10,11,12,14,15,17,18,20,22,25,27,30,33,37,41,45,50,55,61,67,74,82,90};
__u32 csc_bypass_coeff[12] = {0x0400, 0x0000, 0x0000, 0x0000, 0x0000, 0x0400, 0x0000, 0x0000, 0x0000, 0x0000, 0x0400, 0x0000};


