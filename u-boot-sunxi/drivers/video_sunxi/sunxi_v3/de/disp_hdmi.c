...
static struct disp_device_private_data *disp_hdmi_get_priv(struct disp_device *hdmi)
{
	...
	if (!bsp_disp_feat_is_supported_output_types(hdmi->disp,
						DISP_OUTPUT_TYPE_HDMI)) {
...
static s32 hdmi_clk_config(struct disp_device *hdmi)
{
	...
	if (rate_set != rate) {
		for (i = 1; i < 10; i++) {
			rate_parent = rate * i;
			rate_round = disp_sys_clk_round_rate(hdmip->clk_parent,
								rate_parent);
...
static s32 disp_hdmi_set_func(struct disp_device *hdmi, disp_hdmi_func *func)
{
	...
	hdmip->hdmi_func.hdmi_get_video_timing_info =
					func->hdmi_get_video_timing_info;
