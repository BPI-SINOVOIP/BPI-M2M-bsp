...
struct factor_init_data {
	...
	int (*get_factors)(u32 rate, u32 parent_rate,
					struct clk_factors_value *factor);
	unsigned long (*calc_rate)(u32 parent_rate,
					struct clk_factors_value *factor);
...
{
	u32 ret;
    /* return (((unsigned int)factor->priv_regops) ?
     * factor->priv_regops->reg_readl(reg):readl(reg));
     */
	ret = readl(reg);
	return ret;
}
...
{
/* (((unsigned int)factor->priv_regops) ?
 * factor->priv_regops->reg_writel(val,reg):writel(val,reg));
 */
	writel(val, reg);
}
