 ...
 *
 * Flags:
 * SUNXI_PERIPH_NO_GATE - this flag indicates that module gate
 * is not allowed for this module.
 * SUNXI_PERIPH_NO_RESET - This flag indicates that reset
 * is not allowed for this module.
 * SUNXI_PERIPH_NO_BUS_GATE - This flag indicates that bus gate
 * is not allowed for this module.
 * SUNXI_PERIPH_NO_DDR_GATE - This flag indicates that dram gate
 * is not allowed for this module.
 */
...
{
	u32 ret;
    /* return (((unsigned int)periph->priv_regops) ?
     *		periph->priv_regops->reg_readl(reg) : readl(reg));
     */
	ret = readl(reg);
	return ret;
}
static inline void periph_writel(struct sunxi_clk_periph *periph,
		unsigned int val, void  *reg)
{
	/* (((unsigned int)periph->priv_regops) ?
	 *  periph->priv_regops->reg_writel(val,reg) : writel(val,reg));
	 */
	writel(val, reg);
}
