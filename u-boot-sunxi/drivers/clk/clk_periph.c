...
static unsigned long sunxi_clk_periph_recalc_rate(struct clk_hw *hw,
...
static long sunxi_clk_periph_round_rate(struct clk_hw *hw, unsigned long rate,
							unsigned long *prate)
{
	struct sunxi_clk_periph *periph = to_clk_periph(hw);
	struct sunxi_clk_periph_div *divider = &periph->divider;
...
static int __sunxi_clk_periph_set_rate(struct clk_hw *hw, unsigned long rate,
						unsigned long parent_rate)
{
	unsigned long i = 0, factor_m = 0, factor_n = 0, found = 0;
	unsigned long reg;
	...
	if (!div)
		...
		if (div > (div_m<<div_n)) {
			/* WARN(1, "clk %s rate is too large :
			 * %lu\n",hw->clk->name , rate );
			 */
			div = div_m<<div_n;
		}
...
int sunxi_clk_register_periph(struct periph_init_data *pd, void *base)
{
	...
	init.ops = periph->priv_clkops ?
			periph->priv_clkops :  &sunxi_clk_periph_ops;
	init.flags = pd->flags;
	...
	/* fix registers */
	periph->mux.reg =
			periph->mux.reg ? (base + (u32)periph->mux.reg) : NULL;
