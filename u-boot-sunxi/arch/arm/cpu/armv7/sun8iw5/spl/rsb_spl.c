...
static int axp_i2c_read(unsigned char chip, u32 addr, unsigned char *buffer)
{
...
#define AW1667_DCDC5_VOL_CTRL	(0x25)
#define AXP22X_ADDR             (0x68)
...
static int axp22x_set_dcdc5(int set_vol)
{
	...
	if(axp_i2c_read(AXP22X_ADDR, 0x3, &pmu_type))
	{
	...
	if(set_vol > 0)
	{
		...
		if(axp_i2c_read(AXP22X_ADDR, AW1667_DCDC5_VOL_CTRL, &reg_value))
	    {
	    ...
	    reg_value &= (~0x1f);
		/* dcdc5: 1.0V ~ 2.55V 50mV/step */
        if(set_vol >= 1000)
        {
             reg_value |= ((set_vol - 1000)/50);
        }
		if(axp_i2c_write(AXP22X_ADDR, AW1667_DCDC5_VOL_CTRL, reg_value))
		{
...
int set_ddr_voltage(int set_vol)
{
	...
	if(sunxi_rsb_config(AXP22X_ADDR, 0x3A3))
		return -1;
...
{
	if(axp_i2c_write(AXP22X_ADDR, 0xFF, 0x01))
	{
		rsb_printk("write axp failed\n");
		return -1;
	}
	if(axp_i2c_read(AXP22X_ADDR, AXP_ID_ADDR, chip_id))
	{
		rsb_printk("read axp id failed\n");
		return -1;
	}
	if(axp_i2c_write(AXP22X_ADDR, 0xFF, 0x00))
	{
		rsb_printk("write axp failed\n");
		return -1;
	}
