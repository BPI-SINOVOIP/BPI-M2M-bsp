...
static int axp_i2c_read(unsigned char chip, unsigned int addr, unsigned char *buffer)
{
...
static int axp_i2c_write(unsigned char chip, unsigned int addr, unsigned char data)
{
...
static int axp81X_set_dcdc5(int set_vol)
{
	...
	if(set_vol > 0)
	{
	    ...
	    reg_value &= (~0x7f);
		//dcdc5£º 0.8v-1.12v  10mv/step   1.12v-1.84v  20mv/step
        if(set_vol > 1120)
        {
