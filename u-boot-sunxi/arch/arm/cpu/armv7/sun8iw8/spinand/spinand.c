...
__s32  BOOT_AnalyzeSpiNandSystem(void)
{
	...
	{
		spi_nand_setotp(0, 0, 0x18);  //winbond: 0x18,bit3 is BUF mode;
	}
	else if(nand_info.NandChipId[0] == 0x98)
	{
		spi_nand_setotp(0, 0, 0x16);  // toshiba: bit2 is BBI,bit1 is HSE;
	}
	else if(nand_info.NandChipId[0] == 0xa1)// XTX spinand
	{
	...
	{
		spi_nand_setotp(0, 0, 0x10);  //other:0x10,bit3~1 don't care
	}
