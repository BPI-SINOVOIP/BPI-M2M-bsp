...
int auto_update_check(void)
{
	int keyvalue = 0;
	int ret;
	int update_key_max,update_key_min;
#ifdef CONFIG_UDISK_AUTO_CHECK
	int auto_detect_udisk;
	int rtc3_value = 0;
	rtc3_value = sunxi_get_rtc3_flag();
	if(rtc3_value == SUNXI_UDDISK_SPRITE_OVER_FLAG)
	{
		/*clear rtc3*/
		sunxi_set_rtc3_flag(0);
		printf("sunxi clear rtc3\n");
	}else {
		ret = script_parser_fetch("update", "auto_detect_udisk", &auto_detect_udisk, 1);
		if ((!ret) && auto_detect_udisk==1)
			detect_udisk();
	}
#endif
	/*detect key_press*/
	...
	printf("keyvalue = %d\n", keyvalue);
	if(gd->key_pressd_value != 0)
	{
		ret = script_parser_fetch("update_key", "update_key_max", &update_key_max, 1);
		printf("update_key_max = %d\n",update_key_max);
		if (ret)
		{
			printf("cant find auto update_key_max value\n");
			return 0;
		}

		ret = script_parser_fetch("update_key", "update_key_min", &update_key_min, 1);
		printf("update_key_min = %d\n",update_key_min);
		if (ret)
		{
			printf("cant find auto update_key_min value\n");
			return 0;
		}

		if((keyvalue >= update_key_min) && (keyvalue <= update_key_max)) {
...
int detect_udisk(void)
{
	...
	if (ret) {
		...
		uboot_spare_head.boot_data.work_mode = WORK_MODE_USB_UPDATE;
	}
	return ret;
}
...
int fat_fs_read(const char *filename, void *buf, int offset, int len)
{
	...
	if(uboot_spare_head.boot_data.work_mode == WORK_MODE_USB_UPDATE)
	{
...
int sunxi_udisk_update_main(void)
{
	int production_media;
	uchar  img_mbr[1024 * 1024];
	sunxi_download_info  dl_map;
