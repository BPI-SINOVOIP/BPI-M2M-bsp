...
/*should enable CONFIG_FS_FAT*/
#ifdef CONFIG_FS_FAT
	{
		.fstype = FS_TYPE_FAT,
		.null_dev_desc_ok = false,
		//.probe = fat_set_blk_dev,		/*use  fat_register_device*/
		.close = fat_close,
		.ls = file_fat_ls,
		.exists = fat_exists,
		.read = fat_read_file,
		...
		int fstype)
{
	...
	if (fs_set_blk_dev(argv[1], (argc >= 3) ? partition : NULL, fstype))
		return 1;
